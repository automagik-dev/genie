 ...
 */
export function shortPathHash(p: string): string {
 ...
 */
export async function resolveWindowName(sessionName: string, cwd: string): Promise<string> {
...
export async function sessionCommand(options: SessionOptions = {}): Promise<void> {
  // Session is always "genie" unless explicitly overridden
  const sessionName = options.name ?? DEFAULT_SESSION_NAME;
  const workspaceDir = options.dir ?? process.cwd();

  try {
    // If an explicit team name is provided, use it as the window name.
    // Otherwise, derive from cwd basename (with disambiguation).
    let windowName: string;
    if (options.team) {
      windowName = options.team;
    } else {
      // We need the session to exist before we can resolve window names.
      // Check if it exists first; if not, we'll create it below.
      const existingSession = await tmux.findSessionByName(sessionName);
      if (existingSession) {
        windowName = await resolveWindowName(sessionName, workspaceDir);
      } else {
        // Session doesn't exist yet, no collision possible
        windowName = basename(workspaceDir);
      }
    }
    ...
    if (!session) {
      ...
      session = await tmux.findSessionByName(sessionName);
    } else {
      console.log(`Session "${sessionName}" already exists`);
      // Session exists — focus (or create) the window for this folder
    ...
    if (process.env.TMUX) {
      spawnSync('tmux', ['switch-client', '-t', attachTarget], { stdio: 'inherit' });
    } else {
      spawnSync('tmux', ['attach', '-t', attachTarget], { stdio: 'inherit' });
    }
  } catch (error) {

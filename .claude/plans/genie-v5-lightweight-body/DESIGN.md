...
| **D8 (2026-07-02, user-confirmed) — Demolition pulled forward; dev becomes v5 mainline** | Re-sequencing after the foundation shipped: Group 6 (demolition) runs NEXT, before Warp/emit/omni — git history preserves all v4 reference code, so nothing is lost by deleting early, and the user wants the v5→dev PR to be the big deletion. Consequences accepted: (a) omni is temporarily dead on the v5 line (it is PG-broken there anyway once pgserve dies); Group 5 rebuilds it on genie.db from git history immediately after; (b) a `v4` maintenance branch is cut from `dev` BEFORE the merge — it becomes the home of the goodbye banner, the exporter (Group 7), and the final npm release; after the merge, `dev` is v5's mainline. New order: 6 → 3 (Warp/init) → 5 (omni) → 4 (emit) → 7 (exit ramp, on `v4` branch) → 8 (distribution). |

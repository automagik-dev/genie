...
- **The collision snapshot stages everything and keeps only what the install wrote** — detection cannot be narrowed: the snapshot has to copy a foreign same-named skill dir BEFORE `--copy` overwrites it, and at that moment the set of homes this run writes is unknowable, so `collisionCandidateHomes` is `agentSkillHomes(home)` ∪ the previous record's `agentDirs` ∪ the unfiltered bounded `$HOME` walk. Filtering that candidate set instead (the first fix for this defect) lost real user content: `--all` wrote 57 homes on the dogfood host and 53 are outside the four-row table, so a foreign skill in a first-install home or in a home that appeared since the last record was destroyed unbacked. RETENTION is what is filtered, in `finalizeCollisionSnapshot`, once discovery knows what was written: a staged copy whose home is neither proven-written nor a home `--all` targets is deleted (with its mirrored dirs, and the backup root when nothing survives), never reported and never recorded — which is why the 152 unrelated trees (`~/.Trash`, `~/backups`, another product's agent tree) the dogfood host copied into GENIE_HOME and announced as overwrites are gone. When the install does NOT complete (spawn failure, non-zero exit, capped discovery scan) the written set is unknown and nothing staged is discarded: the targeted homes are still named one line each and the remainder gets ONE summary line.

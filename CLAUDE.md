...
- **The two repo-root Orca files are source-only, and never ship in a tarball** — Orca accepts exactly a marketplace source (git repo whose ROOT holds `orca-marketplace.json`) or a plugin source (git repo whose ROOT holds `orca-plugin.json`, or a local folder containing one), so the nested `plugins/genie/orca-plugin.json` alone makes the repo un-installable from git. Root `/orca-plugin.json` is the payload manifest with `main` re-rooted to `plugins/genie/orca-entrypoint.min.js` and nothing else changed; root `/orca-marketplace.json` publishes the single `automagik.genie` entry at ref `main`. `scripts/build-binary.sh` copies neither, so `release-payload-version.ts` must NOT stamp or gate them — their version currency comes from `scripts/version.ts` and the `version.yml` JSON_FILES list (now ten version files), and `release-guard.sh` treats the root manifest as an OPTIONAL version-only child member exactly like the payload manifest. `scripts/orca-manifest-parity.test.ts` is the drift guard. `genie setup --orchestration-mode orca` selects authority only; it never registers the plugin with Orca.
...
- **Codex delivery and activation have separate mutation owners** — install/update may verify, stage, and publish a complete authenticated delivery, but they never advance the Codex cache, change enabled state, reconcile the project route, retire fallbacks, or write Codex roles. `genie setup --codex` requires the matching record before its first prompt or mutation. Once current, it revalidates the authenticated delivery root around marketplace/consent/fallback/role operations, proves one exact enabled plugin before retiring only clean historical fallbacks, and preserves unmanaged, malformed-marker, symlinked, or modified user assets. The deliberately disabled exact-version path stays disabled, skips health and fallback retirement, and still converges the seven managed role profiles. No supported command writes Genie product skills into `~/.agents/skills`.

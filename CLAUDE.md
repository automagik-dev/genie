...
| `mikro` | The mikro microagent runtime, on PATH — `mikro call <agent>` forwards its argv untouched to `scripts/mikro/call.ts` |
...
`call` declares no options of its own: everything after the agent name is
forwarded untouched to `runCallCli` in `scripts/mikro/call.ts`, so a flag added
to the runtime needs no second edit in `src/term-commands/mikro.ts`. Exit codes
are the runtime's — 0 ok, 1 not ok, 2 usage (an unregistered agent name is a
usage error, because the registry in `schemas.ts` owns the name and the answer
schema). The group is exempt from the v4 workspace gate.

...
- **The collision snapshot only covers homes the installer writes** — `collisionCandidateHomes` is `agentSkillHomes(home)` (every home `--all` writes) unioned with the previous record's `agentDirs`, NEVER the unfiltered `$HOME` scan. That scan matched any `<anything>/skills/<name>/` directory under the home, so the dogfood host copied 152 unrelated trees — `~/.Trash`, `~/backups`, another product's agent tree — into GENIE_HOME and printed 152 lines claiming an overwrite that provably never happened.

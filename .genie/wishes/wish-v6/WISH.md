...
- [x] `bun test scripts/wish-workflow-parity.test.ts` proves the hook by probe (amended 2026-09-17, see Scope IN): exit 2 for every forbidden form in every spelling — `gh pr merge`, a `gh api` mutation aimed at a merge endpoint or a protected ref (`-X PUT`, `-XPUT`, `--method=PUT`, the implicit POST of `-f`, a GraphQL merge mutation), `HUSKY=0`/`HUSKY=false`, `-c core.hooksPath=` and `-c core.hookspath=`, `git config core.hooksPath <value>`, `--unset core.hooksPath`, `:main`, `:refs/heads/main`, a bare `git push origin main`, and any of these inside `bash -c`/`eval` — and exit 0 for `gh pr create --base dev`, `git config --get core.hooksPath` (chained or not), a push to `:dev`, a review-thread reply, and any command that only QUOTES a forbidden form as data.

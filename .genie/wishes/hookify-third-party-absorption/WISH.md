...
**Post-merge delta (2026-04-29):** Three Mac CPU follow-ups landed on `dev` after delivery #1: PR #1475 (drop retention from `getConnection`), PR #1476 (`GENIE_SKIP_DB_BOOT=1` in the bun-fallback dispatcher), PR #1481 (narrow inject matchers to `PreToolUse:*` and `PostToolUse:SendMessage` only; persist `syncedSessions` to disk). The wish's plan is unchanged in shape but two implementation realities now apply. (a) Group 3's import logic must consult the renamed `DISPATCHED_EVENT_MATCHERS` constant in `src/hooks/types.ts` — events outside its keys (`PreCompact`, `SessionStart`, `SessionEnd`, `Stop`, `UserPromptSubmit`, `TeammateIdle`, `TaskCompleted`) are NOT forwarded by CC to genie, so foreign hooks on those events MUST NOT be absorbed; they stay in `~/.claude/settings.json` and are reported as `[left-in-place]` rows in `--dry-run`. (b) Any tool that invokes hook code outside `genie serve` (`genie hook test`, scaffold validation) sets `GENIE_SKIP_DB_BOOT=1` to mirror the bun-fallback path's behavior.
...
- [ ] `genie hook import --dry-run` lists every foreign hook in `~/.claude/settings.json` with one of three statuses: `[absorb]` (event in `DISPATCHED_EVENT_MATCHERS`), `[left-in-place]` (un-wired event — would not fire), `[skip — already-absorbed]` (idempotency check). Token Optimizer's `PreCompact`/`SessionStart` entries are reported `[left-in-place]` with rationale.
...
**Deliverables:**
1. `src/term-commands/hook/import.ts` (new) — `genie hook import --from claude-settings`. `--dry-run` (default) emits a settings.json diff + generated `_absorbed/*.ts` list + checksum. `--apply` writes a versioned snapshot at `~/.genie/hooks/_absorbed/snapshots/<ISO>-<sha256>.json` (keep last 10, `latest` symlink, atomic GC), an audit-log entry at `~/.genie/audit/import.jsonl`, rewrites `~/.claude/settings.json`. Idempotency check refuses re-import (post-import hash on settings.json). **Snapshot normalization:** all checksums (snapshot `<sha256>` filename, post-import hash, drift-detection hash) are computed against a stable JSON serialization of settings.json — sorted object keys, normalized whitespace, trailing newline, no BOM. Whitespace-only or key-reorder edits MUST NOT register as drift; a shared helper `serializeStable(obj): string` lives in `src/lib/json-stable.ts` and is the single source of truth used by import / eject / snapshot comparison. Bytes-on-disk are still preserved verbatim in the snapshot so manual round-trips reproduce the operator's original formatting.
...
3. Generated `_absorbed/<plugin>-<event>.ts` template — each absorbed hook is a `defineHook({ name, event, tool, source: 'absorbed', run: <Bun.spawn wrapper that injects captured env + pwd + stdin> })`. Auto-trusted at import time (entry added to `trusted.json` with the file's SHA-256). The wrapper MUST:
   - **Output translation contract:** parse the child process stdout as JSON and translate it into a `HandlerResult` for the dispatcher. Specifically: any `hookSpecificOutput.updatedInput.command`, `modifiedCommand`, or top-level `command` field in the child output is normalized to `hookSpecificOutput.updatedInput.command` (the only shape `executeBlockingChain` at `src/hooks/index.ts:295-298` reads). Unknown output shapes are logged at WARN level via `hook.delivery` span (`data.translation_status: 'unknown_shape'`) and dropped to `undefined` (no rewrite, no error). Token-rewriting plugins like RTK silently losing their savings is the failure mode this contract prevents.
   - **Failure semantics:** wrap the `Bun.spawn` in try/catch. ENOENT (binary missing), non-zero exit, stderr-only output, or any thrown error MUST translate to `return undefined` (implicit allow / no rewrite) — NEVER `decision: 'deny'`. Enforce a per-invocation hard timeout of 5 s; on timeout, emit `hook.delivery` with `data.status: 'timeout'`, kill the child, return `undefined`. The pre-absorption regression risk (rtk crash → all Bash blocked) is the failure mode this contract prevents.
   - **Telemetry:** every wrapper invocation emits a `hook.delivery` span with `data.hook_name`, `data.source: 'absorbed'`, `data.target_command`, `data.translation_status: 'translated' | 'unknown_shape' | 'no_output' | 'timeout' | 'error'`, `data.duration_ms`.
4. `src/term-commands/hook/eject.ts` (new) — `genie hook import --eject`. Verifies current settings.json checksum matches the post-apply hash (using `serializeStable` from deliverable #1); refuses without `--force` on drift; restores the original from the latest snapshot. Removes `_absorbed/*.ts` files and their `trusted.json` entries. **`--force` MUST print a unified diff** of the post-apply settings.json vs the current settings.json before destroying changes, and require an interactive confirmation (or `--force --yes` to skip in scripts). Silent destruction of operator edits is forbidden.
...
- [ ] `genie hook prune --dry-run` lists stale absorbed handlers; `--apply` removes them and updates `trusted.json`.
- [ ] **Output translation contract:** absorbed wrapper round-trips a payload where the child stdout is `{"hookSpecificOutput":{"updatedInput":{"command":"rtk git status"}}}` AND a payload where the child stdout is `{"modifiedCommand":"rtk git status"}` — both emerge as `hookSpecificOutput.updatedInput.command` in the dispatcher result, asserted via `executeBlockingChain` integration test. Unknown-shape stdout produces `data.translation_status: 'unknown_shape'` span and `undefined` result (no rewrite, no error).
- [ ] **Failure semantics:** absorbed wrapper unit tests cover ENOENT, non-zero exit, stderr-only, thrown error, and 5 s timeout — every case returns `undefined`, NEVER `decision: 'deny'`. Each case emits a `hook.delivery` span with the matching `data.status` value.
- [ ] **Snapshot normalization:** import / eject round-trip succeeds when the operator runs `jq . settings.json | sponge settings.json` between apply and eject (whitespace-only edit must not register as drift). Reordering top-level keys via `jq -S settings.json` likewise must not register as drift.
- [ ] **Eject `--force` diff:** invoking `--force` without `--yes` halts and prints a unified diff of post-apply vs current settings.json; with `--yes` proceeds without prompt; in both cases the diff is appended to the audit log.

**Validation:**
```bash
cd /home/genie/workspace/repos/genie && bun run check && bun test src/term-commands/hook src/hooks/env-capture.test.ts src/hooks/absorbed-wrapper.test.ts src/lib/json-stable.test.ts
```

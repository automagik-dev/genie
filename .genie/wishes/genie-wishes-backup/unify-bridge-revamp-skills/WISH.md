...
|-------|-------|-------------|
| 1 | engineer (genie) | Bridge IPC: pidfile + NATS ping + serve lifecycle refactor |
| 2 | engineer (omni) | Turn-monitor fallback config: schema, migration, code path, CLI flag |
| 3 | engineer (genie) | Skills audit: lint script + stale-reference fixes |

### Wave 2 (sequential — depends on Wave 1)
| Group | Agent | Description |
|-------|-------|-------------|
| 4 | engineer (genie) | Delete legacy `genie omni bridge` commands; rewrite status + doctor to use IPC |
...
| NATS subject `omni.bridge.ping` collides with existing Omni subscriber | Low | Pre-implementation grep check is a Group 1 acceptance criterion; evidence captured in the PR. |

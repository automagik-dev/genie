...
- **Closed on Felipe's direction (every handoff writes):** the moment table now covers dispatch (orchestrator comment before checkout) and stale-claim reclaim (comment before the new checkout), so no handoff between orchestrator and engineer, or engineer and engineer, is silent; `report` is gated on the card's current claimant (CLI test: unclaimed and impostor reports are refused), which makes the report tag the trust signal the council asked for.
- **Closed on Felipe's direction (global task state):** the council's "not global" finding rested on the sync's diverged branch discarding one side's events. `syncRoadmap` now unions timeline events by identity when the two sides differ only in `task_events` (new `merged` action, republished snapshot, idempotent; real card or board conflicts still stop), so the conversation reaches every clone and machine through the existing snapshot and hooks. Skill prose names it the global task state; orca mode delegates the same moments to Orca as the lifecycle authority.
- **Deferred with the council's own triggers:** bounding `printTimelineBriefing`. Dogfood evidence (a wish whose every group card shows ≥1 report and ≥1 verdict comment) lands with the next executed wish.

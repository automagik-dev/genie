...
Make the Codex integration introduced by PR #2545 safe to install, update, review, and trust. The original PR has since merged; this branch is a bounded follow-up targeting `dev`, not a retroactive claim that the reviewed PR passed its blocked gates. Implementation has replaced the nine-command hook set, physicalized the skill payload, and hardened ownership/lifecycle paths. PR #2556 merged into `dev` at `a7c3c7ce` and reached `main` via promotion #2557. The successor work then closed both HIGH follow-ups — F42 (`09b368a5`) and F43 (`8e147d87`), each independently reviewed SHIP with the pre-fix failure reproduced — and the seven-lane specialist replay ran at `8e147d87`, the F43 head (4 SHIP / 3 FIX-FIRST lanes; 12 adversarially-verified findings, all code-fixable ones closed in fix loop 1 with a targeted independent SHIP re-review, the rest dispositioned as ledger rows F44–F52 — no full re-replay ran after the fixes, so the shipped head `e3abf2b5` is covered by the re-review, not a re-replay). At head `e3abf2b5` the full aggregate, merge simulation, and read-only live baseline comparison all pass. See the 2026-07-24 closeout below: CI on the exact successor head passed and the metrics routine has gone silent, while independent human approval (F02) was never obtained and stable-release authorization remains blocked by the separate wish.
...
- [x] The Codex hook manifest contains only the three approved behaviors; no retained hook installs software, mutates user/workspace configuration, or repeatedly injects free-form repository text.
- [x] Canonical Codex hook fixtures prove valid JSON output, `apply_patch` decoding, matcher scoping, one-shot Omni evaluation, fail-closed timeout/denial, and portable command launching.
- [x] A source marketplace install and an extracted release plugin both contain every declared Codex skill inside the plugin root, with valid Codex skill metadata.
- [x] `genie update` and uninstall leave the 36 installed adapted user skills and modified role TOMLs byte-identical; Genie-owned clean artifacts still update/remove normally.
- [x] Final-versus-prerelease ordering, identical/stale auxiliary trees, every PR-added auxiliary-tree swap stage, future-dated locks, lost Codex resume threads, blank JSONL output, and option-like prompts have regression coverage.
...
- [x] The final review returns SHIP for the implemented scope (the seven-lane replay ran at `8e147d87` with 4 SHIP / 3 FIX-FIRST and its loop-1 fixes closed under a targeted combined SHIP re-review — not a full re-replay of the shipped head); hooks remain untrusted until the installed cache is refreshed and the user reviews each retained hash in a new task.
- [x] `REVIEW-DISPOSITION.md` has final evidence for every panel/native finding, and no inherited stable-release blocker is mislabeled as closed by this PR.
- [x] `USER-ASSET-BASELINE.md` is unchanged after isolated fixture tests and a final read-only live comparison.

> **Closeout note (2026-07-24):** the Success Criteria above are checked against the consolidated final evidence in [`REVIEW-DISPOSITION.md`](./REVIEW-DISPOSITION.md). The per-group Acceptance Criteria and the QA Criteria further below are left as the historical execution record. Two QA items were **not** met as written: independent human approval before the main merge (see F02) and a release package rebuilt at the successor head (the Darwin ARM64 artifact was only ever built at `d9b0afbd`).
...
| Local Bun is 1.3.9, below the repository minimum | Medium | Record the green local aggregate as local evidence; still require exact-final-SHA CI on supported Bun before ship. |
...
As of that finalization pass (2026-07-11, since superseded by the successor update below): a seven-lane replay on the final tree had not run; exact-final-SHA supported-Bun GitHub CI and independent human approval were pending, and follow-ups F42/F43 were open. The outer bare-Node hook/MCP startup probe is **BLOCKED-UPSTREAM** by the current Codex schema, and the installed hook hashes remain untrusted; neither condition is represented as a local repository pass. Rows F16–F18/F31 remain stable-release blockers regardless of the follow-up verdict.

Successor update, 2026-07-12: PR #2556 merged into `dev` at `a7c3c7ce` and was promoted to `main` via #2557; the remaining work continues on `fix/pr2545-ultra-gate` as a successor PR. F42 was fixed in `09b368a5` and F43 in `8e147d87`, each with an independent SHIP review and an empirical pre-fix reproduction. The seven-lane replay then ran at `8e147d87`: repo-hygiene, QA, and dx-docs returned FIX-FIRST while architecture, security, code-quality, and performance returned SHIP; the 12 confirmed findings (rows F44–F52) were closed in fix loop 1 (`af9fcad1` re-retiring the externally resurrected metrics state, `d1814e40` closing the code findings) with a combined independent SHIP re-review — no full re-replay ran after the fixes. At `e3abf2b5` (dev `e9059f41` merged in) the aggregate passes 1,434 pass / 1 skip / 0 fail, the merge simulation is clean, and the live 36-skill/14-TOML baseline comparison still matches byte-for-byte. The external metrics-updater routine that broke dev CI (F44) was still active at the time of that update.

## Closeout, 2026-07-24

This wish is **DONE**. Successor PR [#2562](https://github.com/automagik-dev/genie/pull/2562) passed **15 checks / 0 failures** on its exact head `bd8c6128`, under CI's pinned Bun 1.3.11 (the repository declares >=1.3.10) — closing **F01**. It was merged into `dev` as `137dede8` on 2026-07-12 and has since been promoted to `main`. **F02 is NOT met as written:** the PR carries one `COMMENTED` review and no `APPROVED` one, and the maintainer who merged it is the same GitHub identity the agent commits were authored under, so no independent approving review exists. The wish closes with that gate recorded as unmet rather than satisfied. Both HIGH follow-ups ship on `main`: F42's guard reaping in `install.sh` and F43's identity-bound uninstall batch in `src/genie-commands/uninstall.ts`.

The F44/F05 operational action is closed **by observation, not by a confirmed disablement**: the external metrics-updater committed daily at ~12:12 UTC from at least 2026-06-06 through 2026-07-14 (all sampled firings authored by the `Claude` scheduler except 2026-07-10's, which is human-authored), its last two firings were manually reverted, and it has produced nothing in the ten days since. The claude.ai routine list queried from the maintainer's account is empty; the account that owned the routine was never identified, so its deletion is inferred from ten days of silence on a daily schedule, not observed directly. `.genie/agents/metrics-updater/**` and the README `<!-- METRICS:START -->` marker are absent from `dev`, and `release-docs.test.ts` continues to fail closed if either returns. Any future metrics automation must avoid both artifacts and open a PR rather than pushing directly to `dev`. If the routine turns out to still exist on an unqueried account, it can re-break `dev` exactly as before — the gate test is the only standing defense.

Standing conditions that outlive this wish and are **not** closed by it:

- Installed hook hashes remain **untrusted**; trusting them requires the user to refresh the installed cache and review each retained hash in a separate task. No hook was trusted during this work.
- The outer bare-Node hook/MCP startup probe remains **BLOCKED-UPSTREAM** by the current Codex schema.
- Inherited rows **F16–F18 and F31** continue to block stable release through [`stable-release-security-gate`](../stable-release-security-gate/WISH.md). Nothing in this wish authorizes a stable release.
- **F02 was never satisfied**: no independent `APPROVED` review exists on #2562. Do not cite this wish as precedent that the independent-approval gate was met.
- No release package was rebuilt at the successor head; the Darwin ARM64 build/extraction evidence is scoped to `d9b0afbd`, and shipped code changed after it.
- Accepted, documented residuals: the doubly-suspended-process lock race (pre-existing, at shell/TS parity), shell PID-reuse conservatism, the role plan-time double-read (benign false-preserve only), and the symlink `readlink`→`unlink` window.

### Current release-gate evidence

| Gate | Evidence | State |
|------|----------|-------|
| Local aggregate | `bun run check` at `e3abf2b5`: 1,434 pass, 1 pre-existing skip, 0 fail (1,435 ran) across 64 files; exit 0 | PASS (local Bun 1.3.9) |
| Complexity ratchet | 2/7 warnings; max 35/42; 0/8 explicit suppressions | PASS |
| Darwin ARM64 package | Build/extraction parity at 21 MB and build SHA-256 `7892e05b…` were recorded at `d9b0afbd`; no package has been rebuilt at the successor head, which changed shipped code | PASS at `d9b0afbd` only |
...
| Final specialist replay | Seven lanes ran at `8e147d87`: 4 SHIP / 3 FIX-FIRST; 12 verified findings closed in loop 1 (`af9fcad1`+`d1814e40`) or dispositioned (F44–F52); combined re-review of fixes SHIP; no full re-replay after fixes | CLOSED (loop 1 of 2) |
| Live user assets | Read-only comparison 2026-07-12: 36/36 skills + 14/14 agent TOMLs byte-identical (one genie-owned product `review` skill sync'd by an explicit live update on 2026-07-12 00:05 — product asset, not user data) | PASS |
| Mergeability | `git merge-tree --write-tree origin/dev HEAD` exit 0, tree `cc6ea068`, vs `origin/dev` `e9059f41`; `git diff --check` clean | PASS |
| Exact-SHA CI (F01) | PR #2562: 15 checks passed / 0 failed on its exact head `bd8c6128`, CI Bun pinned at 1.3.11 (repo declares >=1.3.10) | PASS |
| Independent approval (F02) | #2562 has one `COMMENTED` review and no `APPROVED` review; merged by the same identity the commits were authored under | **NOT MET** — recorded, not waived |
| Metrics-updater routine (F44) | Daily ~12:12 UTC commits through 2026-07-14, then ten days of silence; routine list empty on the maintainer's account, owning account never identified | CLOSED by observation (deletion inferred) |

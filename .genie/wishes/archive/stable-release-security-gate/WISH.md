...
- [x] Production Environment approval requires an independent maintainer and is evidenced without exposing secrets. *(Demonstrated live 2026-07-27 on run 30240023804: automagik-genie dispatched v5.260727.5, the run parked on the `production` gate at "Approve stable release identity", and an independent maintainer approved via the GitHub UI — dispatcher ≠ approver enforced by `prevent_self_review`; evidence is the run's public deployment-approval record, no secrets exposed.)*
...
- [x] Dev/homolog channel releases continue to publish without manual approval throughout and after this work. *(The main-controlled `version.yml` path remains reviewer-free. **Credential setup is now complete (2026-07-25):** the org-wide deploy-key ban made the ssh model unusable, and the fine-grained PAT that replaced it still could not push because the `main` ruleset carried a `pull_request` rule with an empty `bypass_actors` list — every direct push was rejected `GH013` regardless of token scope. PR #2643 replaced both with a per-run GitHub App token (`genie-release-bot`, app id `4393439`, `contents:write` + `metadata:read` only, installed on this repository alone), promoted to `main` in #2644 as `73bd48af`. The App is the sole ruleset bypass actor (`Integration:4393439:always`); no user, role, or Actions bypass was added. **Confirmed 2026-07-26/27:** dev releases v5.260726.12 → v5.260727.4 each advanced `dev.json` reviewer-free via the App token, and the stable run advanced `latest.json`/`homolog.json`/`dev.json` to 5.260727.5 through the same path.)*
...
5. The `release-manifests` environment remains reviewer-free and restricted to `main`; its write credential is provisioned without adding a broader Actions or user-token bypass. **Resolved 2026-07-25 without touching the deploy-key policy:** the credential is a GitHub App (`genie-release-bot`, app id `4393439`) whose short-lived token is minted per run and expires in ~1h, so no long-lived credential can push to `main`. Its private key is held only as the `RELEASE_MANIFESTS_APP_ID`/`RELEASE_MANIFESTS_APP_PRIVATE_KEY` environment secrets, and the ruleset bypass names the App integration alone.

**Acceptance Criteria:**
- [x] Environment approval demonstrably requires an independent maintainer. *(Config evidence: reviewers `namastex888` + `vasconceloscezar`, `prevent_self_review: true`, `can_admins_bypass: false`, exact `main` deployment policy. Demonstrated live 2026-07-27 on stable run 30240023804 (v5.260727.5): automagik-genie dispatched, the run parked on the `production` gate, and an independent maintainer approved — dispatcher ≠ approver enforced by `prevent_self_review`; the release published 36 immutable assets and advanced all three channel manifests.)*
...
- [x] A second maintainer must approve the protected production environment. *(Demonstrated live 2026-07-27 on stable run 30240023804 (v5.260727.5): automagik-genie dispatched, the run parked on the `production` gate, and an independent maintainer approved — dispatcher ≠ approver enforced by `prevent_self_review`; the release published 36 immutable assets and advanced all three channel manifests.)*

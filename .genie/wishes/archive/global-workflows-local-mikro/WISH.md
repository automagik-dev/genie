...
| 8a | AMENDMENT to Decision 8 (2026-09-18, from the Group 4 security review; decided by the orchestrator under the owner's delegation). (i) Fail closed: when the invoking checkout IS a git repository and no trusted ref resolves — no `origin/HEAD`, a stale `origin/HEAD` after a default-branch rename, or a well-formed `--agents-ref` that names no commit — the no-flag path REFUSES any `--dir`, the invoking checkout included, that carries a compared configuration file, at zero cost, naming the ref reason and the remedies (`--agents-ref <ref>`, `git remote set-head origin -a`, `--agents-dir <checkout>/.mikro/agents`). A `--dir` carrying none of them still runs on the shipped agent. The directory comparison with its same-directory exemption survives only on the flag path and outside any git repository. (ii) The compared set MIRRORS the runtime's loader, not a guess: `.mikro/{mikro.yaml,TOOLS.md,SYSTEM.md,CRITERIA.md}` AND the legacy `.rlmx/{rlmx.yaml,TOOLS.md,SYSTEM.md,CRITERIA.md}` that mikro 1.260909.1 falls back to when `.mikro/mikro.yaml` is absent (`config.ts:814-857`); `VALIDATE.md` is excluded because a microagent run overrides it unconditionally from the agent's own directory. The list is one exported constant naming the mikro version and loader it was derived from; it is re-derived whenever the mikro version floor moves. (iii) A compared path that is not a regular file (directory, symlink, dangling symlink) is refused without being read. | The first implementation kept the directory comparison when no ref resolved, which re-opened exactly the hole Decision 8 removes in three common states, and compared only `.mikro/`, while the runtime also loads `.rlmx/` — a PR to any repository without `.mikro/mikro.yaml` could inject `TOOLS.md` (Python in the REPL). Both were reproduced by the reviewer at zero cost. Fail-closed costs an operator one explicit flag in an unusual checkout; fail-open costs code execution. |
...
### 2026-09-18 — Group 1 execution review — SHIP at `50c03be71`

Independent read-only reviewer (Opus, not the engineer). Round 1 at `5d3ed800d`: SHIP with two medium repairs; 7/7 acceptance criteria pass; Validation re-run by the reviewer exit 0, 326 pass / 0 fail; tarball top level exactly the frozen eight, four-file mikro payload, 0 symlinks; hop oracle re-run by the reviewer with the installed binary (a different sha than the candidate) as promoter: `"outcome":"committed"`; compiled bundle outside any git repository with an empty PATH: exit 1, `agentSource: shipped`, one `unavailable:` attempt, ledger under `GENIE_HOME`; no Group 2–5 code leaked. Repairs (fix loop 1 of 2, commit `50c03be71`): the "argv forwarded untouched" claim now names its one exception (genie's global `-V/--version`, `-h/--help`, `--no-interactive`) and a test pins it; the untracked-`.mikro` ledger rule and the agent-name traversal guard are now falsifiable. Round 2 at `50c03be71`: SHIP, no required items; Validation exit 0, 329 pass / 0 fail; both mutants that survived round 1 now fail a named test; the asymmetric `optedIntoMikro` fallback probed under ambient `GIT_DIR`, `GIT_CEILING_DIRECTORIES`, a corrupt index and no git binary — no route from PR content into the in-repository branch; `gitProbeEnv` proven end to end in a linked worktree. Quality pass (security, maintainability, performance): clean.

Carried forward to Group 4 (owner of `scripts/mikro/call.ts` next): the same-directory trust exemption on the no-flag path (Decision 8); the escape wording in `CLAUDE.md` and `mikro.test.ts` should say "prefix the value with a space or use `--prompt-file`" (shell quoting alone does not change the argv token); `delete process.env.GIT_DIR` instead of assigning `undefined` in `call.test.ts`; the pre-existing citation probes (`call.ts:156/183/209`) should use the stripped git environment. Decision 11 wording note: the ledger root is the git toplevel of the cwd, which in a linked worktree is the worktree root, not the primary checkout; behaviour matches the base and `.mikro/runs/` is gitignored.

### 2026-09-18 — Group 2 execution review — SHIP at `af6da53d3`

Independent read-only reviewer (Opus, not the engineer). Round 1 at `c5c6b9f32`: SHIP, 6/6 acceptance criteria; Validation re-run by the reviewer exit 0, 462 pass / 0 fail; `build-binary.sh` re-run: nine workflows staged, no README, no non-regular entries, top level exactly the frozen eight; all three record writers audited for the `workflows` carry-forward; the plugin-era retirement interaction verified in both directions (a later retirement can never remove the newly installed `council.js`); mutation spot-checks kill backup-before-replace (2 tests) and digest-match-before-retire (1 test). Two medium repairs (fix loop 1 of 2, commit `804a684ba`): a recorded name whose on-disk entry became a symlink lost its digest from the record — now every disposition enters the record through one `keepInRecord` helper; the workflows-after-retirement ordering is pinned by a test that fails when the call moves. Round 2 at `af6da53d3` (a conflict-free merge of dev, verified byte-identical to dev's own content): SHIP, no required items; 467 pass / 0 fail; both repairs mutation-checked. Engineer rulings accepted: the channel is synchronous (the plan's Interfaces line said `Promise`; `runManualUpdateConvergence` and the skills channel are synchronous); a missing delivered catalog is `failed` + exit 1; one backup root per run for replaced and retired files.

Chosen behaviours recorded: any consent other than `none` installs the catalog, the same widening as the skills channel (Decision 6 names only `none`); `genie install` has no retirement pass, so there a plugin-era `council.js` is archived as foreign, bytes backed up first. Open advisories, non-blocking: the `try/finally` around `archive.discardIfUnused()` is correct but unpinned (the mutant survives); delivered bytes round-trip through a UTF-8 string before the post-write digest check.

### 2026-09-18 — Group 3 execution review — SHIP at `6f76a5d25`

Independent read-only reviewer (Opus, not the engineer). Round 1 at `a0dac64ac`: SHIP, no required items, 10/10 criteria; Validation re-run by the reviewer (`check:fast` exit 0; doctor 113/0, uninstall 82/0, parity 67/0, `skills:lint` OK); every exit path of `performFreshUninstallPlan` walked — no orphan state in either direction (the workflows sweep runs first and a preserved workflow file keeps the shared record and GENIE_HOME); symlink safety probed, including `rmdir` refusing a symlinked directory; the parity assertions replayed against the six pre-edit skills fail for three independent reasons each. Quality loop (one): the project-path parity assertion was a substring of the user-scope one and could not fail alone — now the literal `<repository root>/...` path, with a self-test of the helper; doctor now names unrecorded files carrying a shipped catalog name (the 2026-09-15 stale-copy shape on a host where the channel has not run), symlinks named and never followed, names taken from the shipped catalog and shared with the installer's reader; uninstall prints the workflows-dir removal even when nothing else was removed. The engineer also found and fixed a red CI on the branch: the new CLAUDE.md gotcha named a retired fossil the drift guard (`src/__tests__/claude-md-drift.test.ts`) forbids; the doc was rephrased, the fossil list untouched. Round 2 at `6f76a5d25`: SHIP, no required items; doctor 117/0, uninstall 83/0, parity 73/0 across nine files, workflows-installer 22/0 (untouched suite over the refactored catalog reader = zero behaviour change), drift guard 52/0; four doctor mutants and one uninstall mutant killed; commit messages re-linted PASS; `SKIP_CI_CHECK=1` gated only the red-branch block its own hook prescribes for a CI fix. Engineer rulings accepted (eight plus six), including: a malformed record prints no workflows line; `(unrecorded)` with nothing to name is a pass carrying the remedy.

Lesson recorded for later groups: a group that edits CLAUDE.md runs the drift guard in its Validation.

### 2026-09-18 — Group 4 execution review — SHIP at `c8a363f01`

Independent read-only security reviewer (Opus, not the engineer), four rounds. Round 1 at `b91d2bddb`: FIX-FIRST. The mechanism held — ref validation, argv-array git calls, `git archive` fidelity proof against `export-ignore`/`export-subst`, disposal, attributes taken from the ref's tree, and three planted mutants each killed the right adversarial test — but two HIGH enumeration gaps were reproduced at zero cost: (1) in a git repository with no resolvable trusted ref (no `origin/HEAD`, a stale one after a rename, or a well-formed `--agents-ref` naming no commit) the no-flag path fell back to the directory comparison and its same-directory exemption, re-opening the hole Decision 8 removes; (2) mikro 1.260909.1 also loads the legacy `.rlmx/` configuration from `--dir`, which was not compared, so a PR to any repository without `.mikro/mikro.yaml` could inject `TOOLS.md`. Plus two MEDIUM: a directory named `.mikro/TOOLS.md` threw EISDIR before `dispose` and leaked the temp agent; the `wish.js` allowlist line lacked `--agents-ref`. Resolution: Decision 8a (fail closed; compared set mirrors the runtime loader, one exported constant with a re-derive rule that also names `MIKRO_AGENTS_DIR` replacing mikro's discovery roots; non-regular compared paths refused unread); fix loop 1 of 2, commit `169550988`, new tests watched failing against the pre-fix code (3 and 2 failures). Round 2 at `69a718870`: SHIP, every reproduction now refused with the boundary unopened; loader swept for any other `--dir`-rooted read — none. One MEDIUM: git FAILING inside a real checkout (`safe.directory` refusal, unreadable index, broken gitlink, no git binary) was indistinguishable from "no checkout" and reached the non-git carve-out. Quality loop, commit `84d02f6a2`: a three-valued `probeCheckout`; only a PROVEN non-repository (git's clean answer and no `.git` found walking up from the cwd) reaches the carve-out. Round 3: SHIP; the walk starts at the cwd, never `--dir`, so PR content cannot influence it, and the classifier is monotone toward closed. Follow-ups, commit `c8a363f01`: the irregular-path refusal re-pinned (the mutant that survived now kills 3 tests), the one probe whose MESSAGE is classified runs under `LC_ALL=C`, and `optedIntoMikro` uses the same three-valued probe so an unanswered checkout never grows an in-tree ledger. Round 4 at `c8a363f01`: SHIP, no required items; 291 pass / 0 fail, drift guard 52/0, `check:fast` OK; no test can reach the real runtime (stub `mikro` first on a scratch PATH). Engineer rulings: the agent still degrades to `shipped` with a reason on a well-formed unresolvable ref (accepted), the configuration does not (the original ruling 2 was rejected); the `.git` walk-up is unbounded (accepted — the safe direction, `--agents-dir` is the escape).

Carried to Group 5 or a later cleanup: `status.ts` / `facts.ts` git spawns on the stripped environment; `bench` over a repository with no `.mikro/agents` should fail upfront; `gitToplevel` is now a test-only export.

### 2026-09-18 — Group 5 execution review — SHIP at `0cc23d151`

Independent read-only reviewer (Opus, not the engineer). Round 1 at `329454b02` (Group 4's final head merged in; the one hand-resolved CLAUDE.md conflict verified byte-clean, nothing of Group 4 reverted): FIX-FIRST, one HIGH and three MEDIUM, all reproduced. HIGH: `genie mikro coach` could not spawn its benches on an installed host — in a real Bun standalone binary `existsSync(Bun.main)` is TRUE for the embedded `/$bunfs` main, so it was passed on as a script (`unknown command`, exit 1, no bench record) while the unit fixture asserted the opposite of the runtime; proven with a compiled probe binary. MEDIUM: commits whose paths git quotes (non-ASCII, quotes) were skipped with a false reason because `git show` lacked `-z`; acceptance criterion 3a had no injected-runner seam and no test; `init` wrote through a symlinked `.gitignore` to a file outside the repository. Fix loop 1 of 2, five commits to `0cc23d151`, new tests watched failing first. Round 2: SHIP, no required items — the reviewer's own compiled probe now yields `[<binary>, 'mikro', 'bench']` with no embedded token and the compile-probe lives in `coach.test.ts` (soft-skips where `--compile` is unavailable); unicode, quote, newline and leading-dash paths, a rename and an add-then-delete all produce correct fixtures; a whole bench round runs against an injected runner across two repositories with the record under the benched one and nothing written in the other; symlink, directory and dangling-symlink `.gitignore` shapes are skipped with the line to add by hand, the outside file byte-unchanged. Validation re-run by the reviewer: exit 0, 410 pass / 0 fail, drift guard and release-docs green, complexity budget unchanged. Engineer rulings accepted, including `mikro-coach` joining the SHIPPED set (never seeded; its `SYSTEM.md` names no genie path; frozen top-level member set unchanged).

Size: the group closed at +2,230 insertions over 19 files against the 2,000-insertion band. The 230 over are the required repairs (tests +197, docs +9, production +66), confirmed as no padding by the reviewer; accepted by the orchestrator rather than splitting a repair away from the defect it fixes.

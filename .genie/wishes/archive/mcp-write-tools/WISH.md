...
| Field | Value |
|-------|-------|
| **Status** | SHIPPED — PR [#2773](https://github.com/automagik-dev/genie/pull/2773) merged to dev 2026-08-13 (PR review SHIP after 1 fix loop, 3/3 groups done); header reconciled + archived 2026-08-24 |
...
Final re-review: **SHIP** — byte-identity finding reproduced independently; probe classification empirically verified under live writer contention (same-process and cross-process; busy rows, never spurious throws) — but the "live writers produce non-stale headers so the probe is skipped" claim recorded here was FALSIFIED by the PR #2773 review: it holds only once a live peer has actually WRITTEN; a live peer on a fully-current database that has not written yet leaves an empty `-wal` and a header the predicate calls stale, so both sidecar guards pass for a HEALTHY, actively-used db (Linux CI: unlinking under a peer's live mmap SIGBUSed that worker and cascaded SQLITE_PROTOCOL to its peers). New invariant: sidecars are never removed without SQLite-proven exclusive access — a `PRAGMA journal_mode = DELETE` round-trip at `busy_timeout = 0`, which any peer that has mapped the wal-index fails instantly (an idle never-queried peer passes, and safely: with no `-shm` mapping there is nothing to SIGBUS, and it re-derives the index on next use — verified by resuming a stale peer post-heal: read OK, write OK, integrity_check ok), and a busy/locked failure skips the heal and propagates the original error; F1-F7 and all pinned surfaces intact; focused suites 83/83; full gate exactly the 2 known environment flakes (Codex doctor 120ms latency budget on this machine; updateCommand unwritable-record-store test); S1→S2 reproduced 4/4 on the pure production path with a writable non-degraded handle and a successful INSERT. Residual LOWs (non-gating, recorded): seam test is a deliberate per-platform pin of the macOS/bun poison shape (may need a platform-conditional on Linux CI); defensive `instanceof BusyDbError` probe mapping errs safe (no recovery on ambiguity).
...
### PR review — PR #2773 — 2026-08-13 — FIX-FIRST → fixed → SHIP (1 fix loop)

PR review at head 0d39908bd (base `dev`): **FIX-FIRST**. Both Codex bot P2 comments verified genuinely resolved (degraded-handle recovery via `promoteDegradedHandle()`, close-before-reopen, e2e green on Linux; blank-group rejection with exact CLI parity vs `v5-task.ts:316`, covered on both surfaces). Linux CI's 5 Unit failures diagnosed: 1 CRITICAL product regression + 2 platform-blind tests. CRITICAL: `removePoisonedSidecars` (introduced 010e6cf6b) unlinked `-shm`/`-wal` under live peers — both guards pass on a healthy, fully-current db whose peers haven't written; catch path condemned on the predicate alone against the module's own invariant; on Linux this SIGBUSed (exit 132) one multi-process worker and cascaded `SQLITE_PROTOCOL` → MalformedDbError to peers (roster/assign/checkout races — all three passed at 029194732, failed at head). HIGHs: `mcp.test.ts` write-protected read test and `genie-db.test.ts` poison-setup pin macOS-only behavior inexpressible on Linux (`SQLITE_READONLY_CANTINIT`). MEDIUM: `SQLITE_PROTOCOL` classified as corruption. LOW: falsified probe claim at this file's Group-1 evidence.

Fix (fixer, 1 loop): `rebuildSidecarsExclusively` replaces the blind unlink — throwaway handle at `busy_timeout=0` must win `PRAGMA journal_mode = DELETE` (SQLite's own exclusivity proof) before any sidecar removal; busy/locked → heal skipped, ORIGINAL error propagates; catch path now requires non-busy throw + header gate + exclusivity proof. Platform escape hatches ported into both tests (real assertions still execute on darwin: 16 and 2 expect() calls). `SQLITE_PROTOCOL` (15) now busy/retryable → BusyDbError. Two new live-peer heal-skip tests pin the regression (verified failing against a restored old `rmSync` body: 2 fail / 10 pass).

Loop 2 (Linux CI at d8b12d6c1 still SIGBUSed 5 multi-process tests; crashing workers died ~20ms into their own startup open): diagnosed TOCTOU in loop 1's heal — after the DELETE-conversion proof succeeds, `db.close()` releases all locks; a peer opens the rollback-mode db and its `applyPragmas` re-enters WAL, creating and mmapping a fresh `-shm`; the healer's post-close `rmSync` unlinks that live mapping → SIGBUS. Fix: the `rmSync` pair is deleted outright — this module never touches a sidecar file; SQLite's own conversion (which unlinks the `-wal` on the same side of the lock boundary as the proof) is the only file operation. Empirically verified: the `-shm` that survives conversion is reinitialized IN PLACE by the retried open's WAL upgrade (same inode before/after, header healed), so the genuine macOS poison still recovers without any unlink. Loop-2 re-review (independent): **SHIP** — no unlink/truncate of sidecars anywhere in src/ production code; rollback-window peer opens cleanly (exit 0, wrote, same inode); 12 processes racing the heal on a genuinely poisoned db → zero signals, integrity_check ok, one clean typed `WalIndexPoisonError`; 355/0 tests, typecheck/lint/complexity clean. Residual LOWs: busy-vs-poison collapse in the rebuild's catch can surface `WalIndexPoisonError` claiming "(-wal holds frames)" under transient contention (typed, retryable, non-corrupting); no darwin test can pin the multi-process unlink race itself — Linux CI is the pin.

Fix loop exhausted (2/2) — escalation diagnosis, 2026-08-13: Linux CI at 4b5c1054f still fails 2 multi-process tests (roster write vs task-create; setTaskWish assign/clear/reassign) with 2/8 workers SIGBUS (bun panic "Bus error … a bug in Bun"), down from 5 failures — and now with ZERO unlinks in our code and no error cascade. All of these tests passed at 029194732 (pre-heal); the sole remaining delta is the heal machinery itself: the virgin≡poison header ambiguity makes the probe fire on healthy contended dbs, and the resulting journal-mode churn (WAL→DELETE→WAL under a live fleet) exercises bun 1.3.11's Linux shm paths in ways plain WAL open/close never does. Cause: **overdesigned-plan** (primary) — the gaps cluster entirely in recovery machinery bolted into the shared open primitive for a narrow macOS-only poison; a simpler design scopes recovery to where the poison arises (the MCP degraded-session path / on actual write failure / doctor --fix) and leaves the shared hot open path churn-free. **env-tool-failure** (secondary) — the crash itself is inside bun 1.3.11's shm handling (CI pin; darwin 1.3.14 local); a bun bump may make the churn survivable but does not remove the design debt. Corrective route: return to wish/brainstorm to remove or rescope the shared-open heal (keeping the independently valuable byproducts already landed: SQLITE_PROTOCOL busy classification, live-peer heal-skip tests, platform test hatches, MCP promoteDegradedHandle recovery — none depend on the shared-open heal). No further fix attempts without that decision; effort_escalations=0/2.

Rescope (human-approved corrective route, 2026-08-13): the heal leaves the shared open primitive entirely — `openSqlite` restored to the exact 029194732 shape (verified against `git show`; only delta on the fleet path is the kept SQLITE_PROTOCOL busy widening) — and lives solely in the MCP write path: `tryWriteOpen` wraps its effective open in `openWithWalIndexRecovery` (one non-test caller repo-wide), covering both the initial `openWriteableDb` and `promoteDegradedHandle` (which is the poison's creator: closing the degraded readonly handle zeroes the `-shm` header on macOS). Unrepairable poison → typed `WalIndexPoisonError` → non-busy → readonly degrade → `read_only_database`; busy carve-out ordering intact. Tests: genie-db.test.ts re-pins "the shared path never heals" (open succeeds, write throws, header stays stale); mcp-tools.test.ts poison test strengthened to a real INSERT + readback + predicate-false (BEGIN IMMEDIATE alone passes on a poisoned handle); falsification proven in both directions (unwire → MCP test fails on disk I/O error; re-wrap shared open → genie-db test fails). Rescope execution review (independent): **SHIP** — scope exact (5 files; mcp.test.ts byte-identical; registries/wire shapes untouched); empirical A/B reproduced (plain openDb: write fails, poison persists; openWriteableDb: heals, write OK); 355/0 focused + 6/0 multi-process ×2 + acceptance e2e green; typecheck/lint/knip/complexity clean; full suite 3253/14 with all 14 in the pre-existing Codex delivery/dogfood family (update.ts /tmp quirk, 0 v5/MCP). Fleet workers can no longer reach probe or churn (structural: their open path is literally the code that passed CI at 029194732); MCP-vs-CLI contention bounded to the safe busy-skip with zero unlinks anywhere. Residual LOWs recorded: genie-db poison test lacks the root (`W_OK`) hatch its new stricter assertions would need under a root container (unreachable on repo runners); no MCP-level pin that `WalIndexPoisonError` degrades rather than nulls if `isBusyError` ever reclassifies it; `WalIndexPoisonError` message still hardcodes "(-wal holds frames)". Final Linux proof: CI on this push.

Re-review (loop 1, independent reviewer): **SHIP** — exclusivity proof probed across 5 peer topologies incl. the decisive cross-process one (blocks with SQLITE_BUSY); idle never-queried peer passes the proof safely (no `-shm` mapping → nothing to SIGBUS; stale peer resumed post-heal: read OK, write OK, integrity_check ok); genuine macOS poison still recovers end-to-end through `openSqlite`; fail-closed contract intact (`SQLITE_NOTADB` → sidecars survive, `MalformedDbError` propagates). Validation: `bun test src/lib/v5 src/term-commands/mcp.test.ts` 355/0; typecheck 0; lint 0 (3 pre-existing warnings); complexity budget intact (mcp.test.ts 36→27). Residual LOWs recorded: WISH.md exclusivity wording (corrected by orchestrator in this commit); journal left in DELETE mode if `rmSync` throws post-mode-change (self-heals via `applyPragmas` next open); refused-foreign db may get WAL→DELETE flip + empty-sidecar removal before `ForeignDbError` re-propagates (no data loss, `walSidecarsEmpty` gate); `WalIndexPoisonError` message hardcodes "(-wal holds frames)" though the rebuild can now fail for liveness. Final proof of the Linux failure-mode closure is CI on this push (darwin cannot reproduce the SIGBUS).

...
- [ ] Visual parity gate: split ratio, theme, and click-to-focus identical to v4. `docs/_internal/tui-host/smoke-matrix.md` carries the before/after screenshots cross-reference + reviewer `Signed-off-by:` trailer.
...
bun install && bun run scripts/tui-spike/xterm-headless-attrs.ts > /tmp/xterm-attrs.txt && grep -q "PASS\|FALLBACK\|OUT OF SCOPE" docs/_internal/tui-host/xterm-attr-coverage.md && grep -q '"@xterm/headless"' package.json
```
...
5. `docs/_internal/tui-host/embed-flag.md` documents the `GENIE_TUI_HOST` flag, its values (`embed` | `legacy`), and the expected default flip in Group 6.
...
- [ ] Visual split ratio + theme byte-for-byte identical to legacy mode (verified by side-by-side screenshots committed to `docs/_internal/tui-host/visual-parity-before.png` and `…-after.png`).
...
**Deliverables:**
1. `docs/_internal/tui-host/smoke-matrix.md` — table per terminal (Warp, iTerm2, Ghostty, Terminal.app, Wezterm, Alacritty, kitty, foot) covering: mount, click-to-focus, drag-select copy, paste, resize, exit cleanup. Each row carries a verdict (PASS / FAIL / WORKAROUND) + the operator screenshot artifact.
2. Microbenchmark execution: run `bun run src/tui/widgets/__benches__/terminal-pane.bench.ts` on Linux + macOS hosts. Output committed to `docs/_internal/tui-host/perf-baseline.md` with p50 / p95 / max emit→render latency and idle CPU sampled over 60 s.
3. Visual-parity screenshots (before = legacy, after = embed) for all eight terminals committed under `docs/_internal/tui-host/visual-parity/<terminal>/{before,after}.png`.
...
bun run src/tui/widgets/__benches__/terminal-pane.bench.ts | tee docs/_internal/tui-host/perf-baseline.md && \
grep -E "^p95.*(emit_render|idle_cpu)" docs/_internal/tui-host/perf-baseline.md && \
...
grep -E "^Signed-off-by: " docs/_internal/tui-host/smoke-matrix.md
```
...
8. Release-notes draft entry in `docs/_internal/tui-host/release-notes.md` (shipped via the same companion docs-submodule PR as the other tui-host docs: [automagik-dev/docs#72](https://github.com/automagik-dev/docs/pull/72)) + a one-line `genie doctor` advisory for v4-on-npm operators who attach into a v5 host. Subsequent public promotion to `docs/v5-launch/release-notes.md` is a follow-up docs-submodule PR after this wish ships.
...
> **Engineer trap warning (added 2026-05-10 after G1 retro; revised
> 2026-05-12 after the convention shift to `docs/_internal/`):** `docs/` is a
> symlink into the `.docs-vendor` git submodule (its own repo,
> `automagik-dev/docs.git`). Writes inside `docs/` land in a SEPARATE repo
> with a SEPARATE PR lifecycle from `wish/tui-opentui-host`. The wish's
> documentation deliverables live at `docs/_internal/tui-host/` (matching
> the project convention established by `docs/_internal/runbooks/release-pipeline.md`
> in commit `24fe5502`). They ship through a **companion docs-vendor PR**
> ([automagik-dev/docs#72](https://github.com/automagik-dev/docs/pull/72));
> the genie PR carries the `.docs-vendor` submodule pointer bump that
> resolves the new path. The G1 spike originally wrote to
> `docs/v5-launch/tui-host/` (wrong submodule path) and the early G2-G5
> phases used the intermediate `.genie/runbooks/tui-host/` location;
> both got swept on 2026-05-12. Do not repeat either mistake.
...
docs/_internal/tui-host/xterm-attr-coverage.md
docs/_internal/tui-host/embed-flag.md
docs/_internal/tui-host/smoke-matrix.md
docs/_internal/tui-host/perf-baseline.md
docs/_internal/tui-host/visual-parity-before.png
docs/_internal/tui-host/visual-parity-after.png
docs/_internal/tui-host/visual-parity/warp/{before,after}.png
docs/_internal/tui-host/visual-parity/iterm2/{before,after}.png
docs/_internal/tui-host/visual-parity/ghostty/{before,after}.png
docs/_internal/tui-host/visual-parity/terminal-app/{before,after}.png
docs/_internal/tui-host/visual-parity/wezterm/{before,after}.png
docs/_internal/tui-host/visual-parity/alacritty/{before,after}.png
docs/_internal/tui-host/visual-parity/kitty/{before,after}.png
docs/_internal/tui-host/visual-parity/foot/{before,after}.png

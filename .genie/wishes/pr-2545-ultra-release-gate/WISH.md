...
| **Promotion state** | PR #2545 merged via `6f682e2b` from promoted source `10ceb2c0`; follow-up PR [#2556](https://github.com/automagik-dev/genie/pull/2556) merged into `dev` at `a7c3c7ce` (2026-07-11) and promoted to `main` via #2557; the F42/F43/replay work after it ships as a successor PR to `dev` |

## Summary

Make the Codex integration introduced by PR #2545 safe to install, update, review, and trust. The original PR has since merged; this branch is a bounded follow-up targeting `dev`, not a retroactive claim that the reviewed PR passed its blocked gates. Implementation has replaced the nine-command hook set, physicalized the skill payload, and hardened ownership/lifecycle paths. PR #2556 merged into `dev` at `a7c3c7ce` and reached `main` via promotion #2557. The successor work then closed both HIGH follow-ups — F42 (`09b368a5`) and F43 (`8e147d87`), each independently reviewed SHIP with the pre-fix failure reproduced — and the seven-lane specialist replay ran at `8e147d87`, the F43 head (4 SHIP / 3 FIX-FIRST lanes; 12 adversarially-verified findings, all code-fixable ones closed in fix loop 1 with a targeted independent SHIP re-review, the rest dispositioned as ledger rows F44–F52 — no full re-replay ran after the fixes, so the shipped head `e3abf2b5` is covered by the re-review, not a re-replay). At head `e3abf2b5` the full aggregate, merge simulation, and read-only live baseline comparison all pass. Exact-final-SHA supported-Bun CI, independent human approval on the successor PR, disabling the external metrics-updater routine, and stable-release authorization remain open.
...
`WISH.md` stays IN_PROGRESS. As of that finalization pass (2026-07-11, since superseded by the successor update below): a seven-lane replay on the final tree had not run; exact-final-SHA supported-Bun GitHub CI and independent human approval were pending, and follow-ups F42/F43 were open. The outer bare-Node hook/MCP startup probe is **BLOCKED-UPSTREAM** by the current Codex schema, and the installed hook hashes remain untrusted; neither condition is represented as a local repository pass. Rows F16–F18/F31 remain stable-release blockers regardless of the follow-up verdict.

Successor update, 2026-07-12: PR #2556 merged into `dev` at `a7c3c7ce` and was promoted to `main` via #2557; the remaining work continues on `fix/pr2545-ultra-gate` as a successor PR. F42 was fixed in `09b368a5` and F43 in `8e147d87`, each with an independent SHIP review and an empirical pre-fix reproduction. The seven-lane replay then ran at `8e147d87`: repo-hygiene, QA, and dx-docs returned FIX-FIRST while architecture, security, code-quality, and performance returned SHIP; the 12 confirmed findings (rows F44–F52) were closed in fix loop 1 (`af9fcad1` re-retiring the externally resurrected metrics state, `d1814e40` closing the code findings) with a combined independent SHIP re-review — no full re-replay ran after the fixes. The external metrics-updater routine that broke dev CI (F44) is still active and must be disabled or re-scoped by the operator. At `e3abf2b5` (dev `e9059f41` merged in) the aggregate passes 1,434 pass / 1 skip / 0 fail, the merge simulation is clean, and the live 36-skill/14-TOML baseline comparison still matches byte-for-byte.
...
| External gates | Exact-final-SHA supported-Bun GitHub CI and independent human approval on the successor PR; disable/re-scope the external metrics-updater routine (F44) | PENDING |

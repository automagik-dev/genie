...
| # | Decision | Rationale |
|---|----------|-----------|
| 1 | The major moves in exactly three authorities; `scripts/version.ts` is the local `npm run version` path only (`package.json:13`) | `version.yml:115` (`prefix=5`, used at `:174-184`) and `:323,329` are independent inline generators |
| 2 | The bump takes effect only when **main** carries it, so it is the last dev change before promotion; merging it fires one final, **valid** `5.x` dev release (corrected 2026-09-19, G1 review H1 — see Group 1 deliverable 3), and #2935 is approved only after that release's `[auto-version]` tip is dev's head; Group 2 lands before Group 1 | `release-guard.sh` loads from the control ref (main), not `source_sha` (`release.yml:91-93`); `version.yml` runs from main. While the 6-major sits on dev it binds nothing but dev's own `release-guard.test.ts`; the hazard is not a rejected tag but the dev-tip push dismissing #2935's approval. An orphaned first-of-major tag cannot be re-run (#2924) |
| 3 | The promotion merge itself mints the first `6.x` tag on main (promotion-tag path, `channel=stable`, awaiting human dispatch); later dev releases then tag `6.x` on the dev path; the first `6.x` stable is `release.yml` `workflow_dispatch` `channel=stable` on the promotion tag | `release.yml:18,22`; `release-guard.sh:116,125` require a human dispatch for stable, and the else branch at `:128-130` rejects a human dev dispatch |
...
- [ ] `genie --help` lists **16** commands — neither `mcp` nor `ui-bridge`; `wish` and `orca` present. No `hire_roster` table, column or snapshot key survives: `grep -rn 'hire_roster' src scripts .genie/roadmap.json .genie/INDEX.md src/lib/v5/TAXONOMY.md` returns only load-bearing residue — the migration's own `DROP TABLE IF EXISTS`, the legacy-compat destructure in `roadmap-sync.ts`, the v1 test fixtures, one completed card's TITLE in `roadmap.json`, and comments recording why the table is gone. `skills/quick/` and `src/lib/legacy-integration-retirement.ts` are absent; a grep for `omni|nats` in `src/` and `package.json` returns only the enumerated historical references.
...
- The Group 1 merge fires exactly one final, valid `5.x` dev release (H1-corrected; see Group 1 deliverable 3); the promotion PR opens immediately after that release's `[auto-version]` tip is dev's head, and no other dev change may land between the Group 1 merge and the promotion merge. `origin/dev` already advanced past `94adc6e96` while this plan was written — that drift is the live form of the first risk below.
...
**Acceptance Criteria:**
- [ ] In **this group's tree** `genie --help` lists **16** commands — neither `mcp` nor `ui-bridge`, with `wish` and `orca` present; `release-docs.test.ts` passes with `Sixteen top-level commands` in CLAUDE.md, `16 CLI commands` in README, and both command tables equal the live registry.
...
- [ ] No `hire_roster` table, column, accessor or snapshot key survives anywhere in `src`/`scripts`. The grep is NOT empty and must not be: the migration cannot drop a table without naming it (`genie-db.ts`'s `DROP TABLE IF EXISTS hire_roster`), `roadmap-sync.ts`'s `stripped()` must still destructure the key so a pre-v6 `roadmap.json` compares, the v1 test fixtures must create the table they prove is dropped, `.genie/roadmap.json` keeps one completed card's TITLE (board history, not state), and the remaining hits are comments recording the removal. `genie task export` no longer carries the key and `task import` of a pre-migration snapshot succeeds.
...
|------|----------|------------|
| The Group 1 merge itself fires one final, valid `5.x` dev release (main's generator is still 5-major), and its `[auto-version]` tip push can dismiss #2935's approval if the approval lands too early — `origin/dev` already moved past `94adc6e96` while this plan was written | High | Decision 2 as corrected (Group 2 lands first); Group 1 merges alone, and #2935 is approved only once the final `5.x` release's `[auto-version]` tip is dev's head with no queued Version run |
...
| The command count moves twice and `release-docs.test.ts` also asserts both tables element-for-element (`:947-948`); deleting `legacy-integration-retirement.ts` strands a host that never updated inside its window | Low | The merge order pins the count: G10 → 17/`Seventeen`, #3019 → 18/`Eighteen`, G6 → 16/`Sixteen`, each group editing counts *and* table rows in its own commit. A retired surface classifies `absent`, the published compat window (`>= 5.260711.6`) has passed, and the manual steps are documented |
...
G1  scripts/{version.ts,release-guard.sh,release-guard.test.ts} .github/workflows/{version,release}.yml   (release.yml: dispatch-input hint only; version-ci-staging.test.ts inspected — no scheme pin)

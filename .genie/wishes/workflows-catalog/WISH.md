...
- **Deferred with the council's own triggers:** the roadmap.json churn decision (task events are exported into the git-tracked snapshot; measure `diverged` verdicts over one dogfood wish before excluding events); bounding `printTimelineBriefing`. Dogfood evidence (a wish whose every group card shows ≥1 report and ≥1 verdict comment) lands with the next executed wish.

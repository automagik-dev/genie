...
# 7. Commit the fix (NO version bump — all hotfixes published as 0.2.1, 0.2.2, etc. at v0.3.0 release, not during hotfix)
   ...
   tools:
     ...
     file_summary: |
       def file_summary(path):
           ...
           # Type validation: path must be a string
...
2. **`scripts/hotfix.sh`** — Hotfix workflow automation with 30min timeout enforcement
   ```bash
   ./scripts/hotfix.sh start gap-001    # stash, branch hotfix/rlmx-gap-001, write START_TIME to .hotfix-timer-gap-001
   ./scripts/hotfix.sh test gap-001     # check timer: if (now - START_TIME) > 1800s, auto-log as DEFERRED, abort
   ./scripts/hotfix.sh finish gap-001   # check timer: if exceeded, error. Otherwise: run tests, merge, pop stash
   # If 30min exceeded: auto-logs gap status as DEFERRED, aborts hotfix branch, pops stash, resumes v0.3
   ```
   **30-minute enforcement mechanism:**
   - `start`: Write timestamp (seconds since epoch) to `.hotfix-timer-gap-NNN` file in repo root
   - `test` and `finish`: Check elapsed time `now - START_TIME`. If ≥ 1800 seconds:
     - Log gap status as DEFERRED in gaps.jsonl
     - Run `git reset --hard` to undo hotfix branch changes
     - Run `git checkout "$CURRENT_BRANCH"` to return to v0.3 branch
     - Run `git branch -D hotfix/rlmx-gap-NNN` to delete hotfix branch
     - Run `git stash pop` to restore v0.3 WIP (with conflict fallback)
     - Exit with error message: "Hotfix exceeded 30min cap, logged as DEFERRED"
   - Cleanup: Remove `.hotfix-timer-gap-NNN` file after finish (success or timeout)

   Includes: branch existence checks, `npm test` before merge, stash pop conflict handling (`git checkout --theirs` fallback), rollback on test failure or timeout.
...
- [ ] `hotfix.sh start` creates branch, stashes work, writes timer file
- [ ] `hotfix.sh` enforces 30min cap: checks elapsed time on test/finish, auto-DEFERs if exceeded
- [ ] `hotfix.sh finish` merges, pops stash, cleans timer file (NO version bump)

...
- **Jaw C — Upstream PR (goodwill only, not v5-blocking — D5):** file a PR on `anomalyco/opentui` wiring the existing `MouseLevel` enum (`packages/core/src/zig/terminal.zig:33-39`) through `setMouseMode`; surface `mouseLevel?: MouseLevel | "none" | "basic" | "drag" | "motion"` in `CliRendererConfig`; add backward-compat shim mapping legacy `useMouse` + `enableMouseMovement` to `MouseLevel`; tests covering each level. Status: PR `anomalyco/opentui#1039` is open + ready-for-review. **Genie does not adopt this in v5 even if it merges** — Jaw A's local override is the permanent v5 solution per D3. The PR exists for the broader OpenTUI community.
...
| D3 | Jaw A (local `\e[?1002l` override) is the **permanent** v5 solution, not transitional. | The override is 30 LOC of well-commented helper code, fully reversible, and depends on nothing outside the genie repo + npm-registry-published `@opentui/core@0.2.6`. Reframed from "transitional workaround" to "the answer" on 2026-05-09 after Felipe pushback: relying on anomalyco's merge cadence is not acceptable, and pinning a fork via git URL costs more in supply-chain risk (Zig toolchain on install hosts, no cosign provenance, fork-rebase maintenance burden) than the override itself. Local override stays even if upstream PR ships. |
...
| D5 | Jaw C (upstream PR `anomalyco/opentui#1039`) is **goodwill only**, not on the v5 critical path. | Originally framed as the "long-term replacement" for Jaw A; reframed 2026-05-09 to courtesy contribution. Genie ships v5 with the local override regardless of #1039's status. If anomalyco merges, we acknowledge it in a follow-up CHANGELOG entry; we do **not** restructure genie's code to depend on it. The PR remains open for the broader OpenTUI community's benefit. |
| D6 | Use the existing `MouseLevel` enum in the upstream PR, not a new `useMouseDrag` flag. | Per D5, this is a courtesy decision for the upstream PR shape. Wiring the existing enum is a smaller, more reviewable PR than introducing a new boolean. Does not affect genie's local override (which doesn't use any opentui config flag — it writes the escape directly). |
...
- [ ] PR opened on `anomalyco/opentui` (goodwill only — not v5-blocking per D5) proposing `MouseLevel`-through-`setMouseMode` + `mouseLevel` config surface, with tests for each level + backward-compat shim. PR URL recorded in this wish's Review Results. PR converted from draft to ready-for-review; merge timeline at anomalyco's discretion.

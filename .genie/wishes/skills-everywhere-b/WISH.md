...
| 10 | All `doctor.ts` surgery happens in G3, including the Kimi and `agent sync:` checks whose payloads G4/G5 delete later — **amended 2026-08-31 (G4 fix loop 1): one exception, the `omni hook-timeout` check, is G4's** | Deleting a caller never requires deleting the callee first, and one owner for a 2,711-line command file is worth more than surface-aligned grouping. The exception is the inverse case: `checkOmniHookTimeout` (with `CcSettings`/`HookCommand`/`HookMatcher`, `minMatchingTimeout`, `findDispatchHookTimeoutSec`, `evaluateOmniHookTimeout` and its registration) reads `~/.claude/settings.json` for a **`hook dispatch`** PreToolUse timeout. G4 deletes the dispatch runtime that could ever be registered there, so on the G4 head the check can only emit advice about a command that no longer exists. It leaves with its subject, together with the `omni hook-timeout guardrail` describe in `doctor.test.ts`. |
...
5. Delete the Codex **role-agent transaction engine** from `src/lib/runtime-integrations.ts`: the `CODEX_AGENT_*` prefixes (`:1123-1126`), the inventory writer around `:709,881,1208,1246-1248`, and the `plugins/genie/codex-agents/*.toml` byte-binding and bundle-root discovery at `:554,571,723,2308` — all of which break the moment `codex-agents/` is deleted. **Keep** `inspectCodexAgentOwnership` and `removeCodexPluginRegistration`: Group 2's retirement module imports both (`legacy-integration-retirement.ts:85`).
    ...
    **Amended 2026-08-31 (G3 fix loop 1): only the payload-forced half lands here; the transaction engine is transferred to Group 5 deliverable 3.** Done in G3: the `codex-agents/*.toml` byte-binding, the parity test that bound `CANONICAL_CODEX_ROLE_AGENT_DIGESTS` to the deleted TOMLs, and the bundle-root discovery (`isBundleRoot` now probes `plugins/genie`). Transferred to G5 because excising them here cascades into ~20 unrelated `runtime-integrations` symbols that the Claude-plugin convergence G4/G5 own: `CODEX_AGENT_INVENTORY_NAME` / `_OWNER` / `_MODE`, the `CODEX_AGENT_*` transaction prefixes, the inventory writer, `installCodexAgents`, `convergeCodexPlugin`, `convergeCodexPluginOnly` and `installCodexIntegration`. All of them are already **unreachable in production** on the G3 head — `install.ts`'s `claudeOnlyScope` keeps Codex out of `installRuntimeIntegrations` and `update-integrations.ts` returns `[]` for a Codex selection — so the deferral carries no runtime hazard; they survive `knip` only because `runtime-integrations.test.ts` still exercises them.
...
7. **Rewrite `genie uninstall`** (3,731 lines today). New contract: (a) delete the recorded inventory skill dirs from the recorded `agentDirs`, honoring `isSafeSkillName` and refusing anything that is not a real directory it recorded; (b) remove the install record; (c) run `runLegacyIntegrationRetirement` for leftovers; (d) remove the binary and the existing v4/legacy collectors, **including** the classifier-only `.codex/skills/.curated` collector — a legacy directory a very old Genie may have left, which uninstall still collects and sync must never recreate; (e) drop the whole `agent-sync.js` import block (`uninstall.ts:35-60`) and the flat-agent-transaction machinery. Foreign skills in the same dirs stay untouched, and Group 1's `collisions[]` are reported with their backup root, not restored.
    ...
    **Amended 2026-08-31 (G3 fix loop 1): (a)–(d) land in G3; (e) is transferred to Group 5 deliverable 2.** `uninstall.ts` still imports `agent-sync.js` — including `runFlatAgentTransaction` — because that module only leaves in G5, and pre-emptively inlining or duplicating its primitives here would mean writing code G5 deletes. What G3 does deliver and prove: the Codex lifecycle lease and the `codexLifecycleBusyTrailer` are gone, `runLegacyIntegrationRetirement` runs backup-first inside the confirmed destructive path only, the `.codex/skills/.curated` classifier-only collector is preserved, and the record-driven contract is covered by tests (foreign skills in a recorded dir survive; a recorded name that is a symlink, a file, or absent is refused; a traversal record is rejected wholesale). G5 owns the import-block removal, and its acceptance criterion "`git grep -rn "agent-sync" -- src scripts plugins package.json` returns nothing, prose included" is what enforces it — the mechanical break is the mechanism, not the guarantee.
8. Delete `plugins/genie/.codex-plugin/plugin.json`, `plugins/genie/codex-agents/*.toml` (7 role profiles), `plugins/genie/references/codex-integration-map.md`, and root `.agents/plugins/marketplace.json` — **and, in the same commit** (Decision 11), delist them from `scripts/build-binary.sh:118-120,131`, `scripts/release-payload-version.ts:19,39` plus `verifyCodexMarketplaceEntry` itself (`:90-101`) and its call sites (`:127`, `:180`), `scripts/version.ts:201`, `scripts/release-guard.sh:164`, `.github/workflows/build-tarballs.yml:71` (the `.agents/plugins/marketplace.json` path filter), and the `.agents` entry of `AUX_LAYOUT_DIRS` (`install.ts:78`, `update.ts:3666-3672`). Fix whichever of `scripts/release-payload-version.test.ts`, `version-format.test.ts`, `version-ci-staging.test.ts`, `release-docs.test.ts` and `runtime-integrations.test.ts:359` the edit breaks; if an assertion binds `version.ts` and `version.yml` together, do both in the same commit.
    ...
    **Added 2026-08-31 (G3 fix loop 1) — the delisting is not complete without the installer's exact-member allowlist.** `src/lib/install-promotion.ts:32`'s `INSTALL_PAYLOAD_MEMBERS` is an **exact** set (`verifyPayloadLayout` compares length *and* every sorted name, `:1786-1792`), so the moment `build-binary.sh` stops staging `.agents` every real tarball has seven members against an eight-member allowlist and `install.sh`'s promoter aborts with `staged install does not match the exact installer member allowlist` — a release-breaking failure invisible to `bun run check`, to `build-binary.sh` and to `fresh-install-smoke`, which never exercise `install-promote`. Drop `.agents` from `INSTALL_PAYLOAD_MEMBERS` in the same commit, re-point the payload fixtures that build the eight-member tree (`src/lib/install-promotion.test.ts:54` and its per-member interruption scenarios, `scripts/install-swap.test.ts:77,215`, `src/genie-commands/install-promote.test.ts:47`, `src/genie-commands/__tests__/update-command-publication.test.ts:48,51`, `tests/support/update-current-boundary-runner.ts:14,17`), and prove it end to end with `tar -tzf` over a freshly built tarball: the top-level member set must equal `INSTALL_PAYLOAD_MEMBERS` exactly. `src/genie-commands/install.test.ts:766-776` stays as-is — it is the *negative* assertion that an extracted `.agents/` is no longer a managed layout dir.
...
**Acceptance Criteria:**
- [ ] `git grep -n "codex-activation\|codex-lifecycle-lease\|codex-delivery\|codex-rollback\|codex-host-observation\|codex-doctor-observation\|codex-lifecycle-truth\|codex-release-version" -- src scripts` returns **no module reference**: every remaining hit is classified in the PR as one of exactly two non-importer classes, and any third class is a regression. **(i) Negative guards** — `src/genie-commands/setup.test.ts` asserts these tokens are *absent* from `setup.ts`. **(ii) Wire contracts** — the `'genie-codex-activation-tree-v1\0'` digest domain separator, which `scripts/build-delivery-evidence.ts` and `src/lib/release-payload-proof.ts` must keep byte-identical on both sides; renaming it would invalidate every already-signed evidence pack, so it stays until a deliberate domain rotation. G7 re-runs this grep against the same two-class list, not against "returns nothing".
- [ ] `genie update --publish-local-delivery` still parses, still gates on `GENIE_RELEASE_DOGFOOD`, and terminates with **exit 0 / `code: 'delivery-verified'` / `deliveryComplete == true`** on the smoke's fixture. **Amended 2026-08-31 (G3 fix loop 1), was: exit 2 / `activation-pending`.** The old terminal meant "delivery published, activation still pending"; with the Codex activation store deleted there is no record left to publish and no activation left to defer, so the surviving work — `verifyDownloadedDeliveryEvidence` over artifact digest, manifest, DSSE and caller observation — succeeds or fails on its own, and a non-zero exit would assert a pending step that cannot exist. `install.sh:65`'s `INSTALL_ACTION_REQUIRED_TRAILER` is proven unreachable by this change, so no operator sees a false "activation deferred" message. `bun test src/genie-commands/local-delivery-repair.test.ts scripts/workflow-yaml-parse.test.ts` green, with `workflow-yaml-parse.test.ts:173` and `:180` unchanged and `:175`/`:177` moved with the terminal.
...
- [ ] Every surviving doctor check is named in the PR with the module it depends on; **`doctor --json` drops the `integrationSummary` key entirely** (`git grep -n integrationSummary -- src` empty) — **amended 2026-08-31 (G3 fix loop 1), was: "emits `integrationSummary` only for surfaces that still exist"**. Every surface it summarized (Codex plugin, role agents, agent sync) leaves with this group, so narrowing it would leave an always-empty object; its one remaining reader is `scripts/validate-live-dogfood-evidence.ts:227,329-330`, which G6 deletes with the rest of the dogfood gate. `bun test src/genie-commands/doctor.test.ts` green.
...
- [ ] **Added 2026-08-31 (G3 fix loop 1).** The built tarball's top-level member set equals `INSTALL_PAYLOAD_MEMBERS` exactly — paste `tar -tzf dist/genie-<version>-<platform>.tar.gz | sed 's|^\./||' | awk -F/ 'NF>0{print $1}' | sort -u` beside the constant — and `bun test src/lib/install-promotion.test.ts scripts/install-swap.test.ts src/genie-commands/install-promote.test.ts` is green. Anything that stops shipping a payload member must delist it here in the same commit (see deliverable 8).
- [ ] No workflow job references a file this group deleted: for each deleted basename, `git grep -n "<basename>" -- .github` is empty.

**Knowingly red from this group until Group 6 — release freeze.** This group deletes `setup --codex` and moves the `--publish-local-delivery` terminal, but `.github/workflows/release-publish.yml:728` still invokes `tests/support/codex-dogfood-{harness,entry-runner}.ts`, whose deletion Decision 7 assigns to **G6 deliverable 7**. The harness cannot pass on this head: it requires exit 2 from `update --publish-local-delivery` (`codex-dogfood-harness.ts:1490`), a `repairTrailer.code === 'activation-pending'` (`:1498-1502`) and a `setup --codex` run matching `/Activated Codex plugin/` (`:1521-1533`), and `scripts/validate-live-dogfood-evidence.ts:430-434` additionally requires the `activation-pending` trailer plus the `integrationSummary` key doctor no longer emits. Therefore, from the moment G3 merges into `wish/skills-everywhere-b` until G6 lands: **`codex-native-dogfood` and its `codex-dogfood-completeness` roll-up are expected red**, `codex-dogfood-completeness` remains a `publish.needs` edge (`release-publish.yml:1621,1631`), and **no release may be cut from `wish/skills-everywhere-b` or from any branch carrying G3 without G6**. This is a sequencing consequence of the plan, not an execution defect; G6 deliverable 7 closes it by deleting both jobs and the harness in one commit.
...
2. Delete the Claude plugin payload: `plugins/genie/.claude-plugin/plugin.json`, `plugins/genie/agents/*.md` (7 role agents), `plugins/genie/hooks/{hooks.json,codex-hooks.json}`, `plugins/genie/workflows/council.js`, `plugins/genie/rules/genie-orchestration.md`, `plugins/genie/scripts/**` (all seven executables plus `validate-wish.cjs`/`.test.ts`, `statusline.sh` and `src/`), `plugins/genie/skills/**` (the mirror), `plugins/genie/{settings.json,genie.ts,index.ts}`, `plugins/genie/references/**` **except** `orca-orchestration.md`, and root `.claude-plugin/marketplace.json`. **Keep `plugins/genie/plugin.json`** (it carries `extensions."dev.orca.compatibility"`), **`plugins/genie/package.json`** (version-stamped, license-asserted at `release-docs.test.ts:67,721`) and **`plugins/genie/README.md`** (read 6× in `release-docs.test.ts` — `:822,943,961,963,974,1006`) — the README is rewritten to Orca-only content, and the three content assertions at `:961,963,1006` plus the two composite reads at `:943,974` are re-pointed to whatever the rewrite says. **Amended 2026-08-31 (G4 fix loop 1): "re-pointed" means the rewrite's own contracts are pinned, not merely that the Codex-era ones are dropped.** The Codex-era claims (`the only Genie-managed skill provider`, `36 adapted skills`, `no Codex network lookup`, the plugin-cache/`~/.agents/skills` negatives) are gone because the Orca-only README no longer makes them; in their place `release-docs.test.ts` pins five statements the new README does make — `No launcher or registration ships`, the `[Orca dual-mode operator and contributor contract]` link and its `references/orca-orchestration.md` target, plus the three fresh contracts `bun run lint:orca-bundle` (entrypoint byte-binding), `2000 files / 50 MB` (Orca's symlink-free install cap) and `Genie never registers the plugin with Orca`. A rewritten README with no drift guard is not an acceptable outcome of this deliverable.
...
4. Delete `scripts/council-workflow-lint.ts`, `scripts/hook-bundle-parity.ts`, `scripts/hook-budgets-lint.ts`, `scripts/hook-content-binding.ts`, `scripts/plugin-executables-check.ts`, `scripts/sync-plugin-skills.ts` with their tests and `scripts/fixtures/plugin-executables/`, plus `src/lib/council-workflow-stamp.test.ts` (it `require`s `plugins/genie/scripts/council-stamp.cjs` at `:20`) and `src/lib/wish-status.test.ts`'s dependency on `plugins/genie/scripts/session-context.cjs` (`:15`). **There is no `scripts/council-workflow-lint.test.ts`** — only `council-workflow-lint.ts` exists, so "with their tests" covers the five that have one; do not chase a sixth.
    ...
    **`scripts/fresh-install-smoke.ts` is the shared consumer of two of these deletions, and both cuts belong to *this* group** (it is invoked by `build-binary.sh:58`, so a dangling import here breaks every tarball build and both this group's and Group 6's acceptance runs of that script):
    ...
    - `:33` `import { SHIPPED_SKILL_NAMES, assertPluginSkillsInSync } from './sync-plugin-skills.ts'` — `SHIPPED_SKILL_NAMES` (`sync-plugin-skills.ts:36-62`, **25** entries at `dev` head after #2870) must be re-pointed at `skills/*/SKILL.md` rather than deleted into a dangling import, and `assertPluginSkillsInSync` removed with its **two real call sites** — `version.ts:250` (a function call, imported at `version.ts:40`) and `fresh-install-smoke.ts:493`. **`build.js:54` is not a call site**: it is `execFileSync('bun', [.../sync-plugin-skills.ts, '--check'])`, a subprocess spawn, and `build.js:55` spawns `fresh-install-smoke.ts`, which survives — so `build.js` needs the `:54` line deleted and the `:55` line left alone. (G6 deletes `build.js` outright; this group must still not leave it spawning a script that no longer exists.)
        ...
        **Amended 2026-08-31 (G4 fix loop 1) — "re-pointed" must stay enforceable against the payload.** The rehomed inventory is the set of `SKILL.md`-bearing directories under the repository `skills/` tree, and the smoke compares the tree it was pointed at against that set **on every run, `--skills-dir` included** — those are exactly the staged (`build-binary.sh:113`) and extracted (`:190`) payload trees, which is where a dropped skill would actually appear, and where the deleted per-skill mirror loop (`build-binary.sh:141-148`) used to look. A comparison that is skipped whenever `--skills-dir` is passed compares `skills/` to itself and can never fail; that is the deleted form of `SHIPPED_SKILL_NAMES`, not the re-pointed one. The only opt-out is the explicit `--allow-partial-inventory` flag, used solely by `fresh-install-smoke.test.ts`'s synthetic fixture trees and by no release path, and `fresh-install-smoke.test.ts` owns a test that drops one skill from a copy of the real tree and asserts the failure.
5. Strip the plugin-scripts coupling from files that survive: `src/lib/agent-sync.ts:4794` (council-stamp byte-parity) and `:519,5555,5762,6476` (plugin-root discovery) plus `agent-sync.test.ts:88`; `.github/workflows/build-tarballs.yml` path filters `:62` (hook-bundle-parity), `:64` (hook-content-binding), `:65` (plugin-executables-check), `:66` (sync-plugin-skills) and `:72` (`.claude-plugin/marketplace.json`) — `:63` `orca-bundle-parity`, `:67` `fresh-install-smoke` and `:68` `skills-lint` stay; **all five sibling gate steps in `.github/workflows/ci.yml`**, not just one — `Council workflow lint` (`:120-121`), `Generated hook bundle parity` (`:123-124`), `Codex hook launcher content binding` (`:129-130`), `Shipped plugin executable static checks` (`:132-133`) and `Plugin skill mirror + Kimi command orphan check` (`:135-136`), leaving `Native Orca bundle parity` (`:126-127`) and `Fresh-install smoke` (`:138-139`) in place; `scripts/build-binary.sh:57,59,61,62` and its required-file entries `:129-130,135-137` and per-skill mirror assertions `:141-148`; `scripts/build.js:54,123-124,132-146`; `biome.json:56-57`; `.coderabbit.yaml:22`. Delist `plugins/genie/.claude-plugin/plugin.json` and `.kimi-plugin/plugin.json` from `release-payload-version.ts:18,20,38,40`, `version.ts:200,202,207`, `release-guard.sh:163,165` and `version.yml:196-198` (Decision 11), and remove `assertPluginSkillsInSync()` from `version.ts:250` and `build.js:54`.
    ...
    **Amendment 2026-08-31 (G4 fix loop 1) — deleting root `.claude-plugin/marketplace.json` (D2) is a change to the *installer payload contract*, not only to the five toolchain lists.** Same shape as the `.agents` defect G3 found in its own fix loop, and invisible to `check`, to `build-binary.sh` and to `fresh-install-smoke`: `install.sh`'s promoter compares the staged tree against an exact member allowlist, so a tarball built without `.claude-plugin` is rejected unless the allowlist moves with it. The full set this group must edit in the same commit, all done and proven: `src/lib/install-promotion.ts:32` `INSTALL_PAYLOAD_MEMBERS` (eight names → six) and `:191` `EXPECTED_MEMBER_KINDS`; `src/genie-commands/install.ts:52` `AUX_LAYOUT_DIRS`; `src/genie-commands/update.ts:1278` `AUXILIARY_DELIVERY_TREE_NAMES` and `:3219` `syncAuxiliaryContent`'s target list; plus the five toolchain lists (`release-payload-version.ts`, `version.ts`, `release-guard.sh:163-183`, `version.yml:188-246` — whose "expected exactly **eight** version files" error becomes **five** — and the `build-tarballs.yml:72` path filter). Evidence: `tar -tzf` over the freshly built tarball shows a top-level set of exactly `LICENSE VERSION genie plugins skills templates`, byte-equal to the new allowlist, with `scripts/install-swap.test.ts` and `src/lib/install-promotion.test.ts` green. `version.yml` is `workflow_run`, so its edit is inert on `dev` until promoted (G6 D11's rule, inherited here).

    **Open item handed to Wish C (not fixed here):** `$GENIE_HOME/.claude-plugin`, left on hosts by any install before this wish, is now neither synced nor removed by `genie update` — `syncAuxiliaryContent` no longer names it, and `LEGACY_INTEGRATION_SURFACES` (`src/lib/legacy-integration-retirement.ts:118-136`) has no surface for it. `genie uninstall` is unaffected (it removes `GENIE_HOME` wholesale), so the residue is update-path only and inert. Wish C decides whether the retirement module owes it a surface, exactly as it did for `.agents`.
...
**Acceptance Criteria:**
- [ ] `git grep -n "src/hooks\|hook dispatch\|CLAUDE_PLUGIN_ROOT\|LENS_ROOT" -- src scripts package.json` returns **no importer of anything this group deletes**; every residual hit falls into exactly one of two named classes, each hit classified in the PR — **amended 2026-08-31 (G4 fix loop 1), was: "returns nothing"** (same shape as G3 AC 1, amended in G3 fix loop 1 for the same reason: the four tokens are also the vocabulary of the negative guards that prove the deletion, so a literally-empty grep would require deleting the proofs).
    - **(a) negative guards and banned-token lint rules** — `scripts/skills-lint.ts:204,265` (the `CLAUDE_SKILL_DIR` / `CLAUDE_PLUGIN_ROOT` banned-token rule that keeps skills tier-portable) and its test at `scripts/skills-lint.test.ts:42`; `scripts/fresh-install-smoke.ts:270` (`CLAUDE_PLUGIN_ROOT: undefined` — the smoke *unsets* the variable); `src/genie-commands/legacy-v4.test.ts:164` and `src/lib/claude-settings.test.ts:52` (settings fixtures carrying a `genie hook dispatch` entry, which is exactly what v4 residue detection and settings-hook removal must still recognize on a host installed before this wish).
    - **(b) the surviving council-template stamper, G5-owned** — `src/lib/agent-sync.ts:141,4657` and its fixtures in `agent-sync.test.ts:82,86,246,2427,2613`, `legacy-integration-retirement.test.ts:47`, `uninstall.test.ts:889,908,930,1059,1074,1096` and `__tests__/update.test.ts:2866`. `LENS_ROOT` here is the `/council` workflow template placeholder, not a hook path; `agent-sync.ts` and its test leave in **Group 5 deliverable 2**, which is where this class goes empty.
...
- [ ] `bun scripts/fresh-install-smoke.ts` runs standalone with no import of `hook-content-binding.ts` or `sync-plugin-skills.ts` (only `./skills-lint.ts` remains), and `bun test scripts/release-docs.test.ts` is green. **Added 2026-08-31 (G4 fix loop 1):** the rehomed shipped-skill inventory is enforced against the tree actually scanned — including both `--skills-dir` payload runs inside `build-binary.sh` — with `--allow-partial-inventory` as the only opt-out and a `fresh-install-smoke.test.ts` case that proves a payload missing a skill fails.
...
- [ ] `git grep -n "lint:council-workflow\|lint:hook-\|lint:plugin-" -- . ':!*.md' ':!.genie/wishes'` returns nothing, including `.github/workflows`, `.husky` and `package.json` — **amended 2026-08-31 (G4 fix loop 1), was: `-- . ':!*.md'`**. The excluded path holds four hits in one **shipped** wish's own validation evidence (`.genie/wishes/council-workflow/validate/g2-engine.sh:33`, `g3-cutover.sh:22`, `g5-gate.sh:5,9`): they are ledger artifacts of a closed wish, not gates — nothing in `.github/workflows`, `.husky` or `package.json` invokes them, verified by grep. Each of the three carries a `retired by skills-everywhere-b G4` header so a re-run is never mistaken for a live gate; their bodies stay byte-unchanged, because rewriting a shipped wish's evidence would falsify it.
...
2. Delete `src/lib/agent-sync.ts` (6,733) and `src/lib/agent-sync.test.ts` (5,305), plus `scripts/generate-codex-fallback-allowlist.ts` (imported at `agent-sync.test.ts:49`; its other consumer left with Group 3). By this point the only remaining non-shim importer is `update.ts:23`'s `runAgentSync` — delete that call chain (`update.ts:1764-1810`, `:2702-2799`, `:2873`) including the `~/.genie/.last-agent-sync` throttle marker and the `agent-sync: no genie plugin source found` probe at `:2783`, and delete `src/lib/ordered-lifecycle-leases.ts` or its remaining agent-sync coupling if nothing else uses it. **Inherited from Group 3 deliverable 7(e) (transferred 2026-08-31, G3 fix loop 1):** the same commit drops `src/genie-commands/uninstall.ts`'s whole `agent-sync.js` import block (`:35-60`, `runFlatAgentTransaction` included) and the flat-agent-transaction machinery it feeds, re-proving the record-driven uninstall contract G3 tested (foreign skills survive; symlink / file / absent / traversal records refused) against the slimmer file rather than treating the compile break as sufficient evidence.

    **Amended 2026-08-31 (G5 fix loop 1) — what `--sync-only` means after the engine is deleted.** Deleting `update.ts:1764-1810` removes `runLegacySyncOnlyConvergence`, whose header recorded the ratified contract "legacy `--sync-only` is a pure agent-sync compatibility path". The flag, its help text and the `GENIE_UPDATE_SYNC_ONLY=1` env contract survive (`genie.test.ts` pins them), so the mode needs a body. It becomes `runTrackedManualUpdateConvergence(VERSION)` — the ordinary post-delivery convergence: skills channel → Claude plugin refresh → plugin-era retirement, still branching before every manifest fetch, download and binary swap. **This widens the mode**: on the base it ran no skills install and no retirement, so `genie update --sync-only` now performs a backup-first destructive retirement of plugin-era host assets it never performed before. Accepted rather than the two alternatives — a deprecation no-op (leaves the one surface an operator uses to converge a client home doing nothing, on the exact release that retires those assets) or a retirement-narrowed convergence (a third semantic, needing new machinery in `runManualUpdateConvergence`, and it would let a `--sync-only` host install the replacement skills while leaving the plugin-era originals beside them — precisely the double-state wish decision 2 forbids). Retirement is backup-first, idempotent, gated on `skills.status === 'installed'`, and identical to what a plain `genie update` on the same head performs. **The uninstall journal generation bumps v3 → v4 in the same commit** (deliverable 2's scope change removes `agentAssets` / `codexRoleAgents` / `codexRoleInventoryStatus` from a `.strict()` scope): v3 must therefore join v1/v2 in the authenticate-then-discard-then-re-record legacy lane, in `readUninstallBatchDecision` **and** in `discardLegacyUninstallBatchDecision`'s guard, with a migration test and a tampered-v3 fail-closed test — v3 is the generation every shipped binary wrote, so an interrupted-uninstall host is the common case, not the rare one.
3. **Slim `src/lib/runtime-integrations.ts`** (4,348). Keep, with a named consumer for each: `IntegrationSelection` and the consent read/write (`install.ts`, `update.ts`, `skills-installer.ts:44-49`); `CommandRunner` / `CommandResult` / `runBoundedIntegrationCommand` (`skills-installer.ts`); `removeCodexPluginRegistration` + `planCodexPluginRegistrationRemoval` and `inspectCodexAgentOwnership` (`legacy-integration-retirement.ts:85`); `inspectRuntimeIntegrationEvidence` (retirement acceptance + doctor); the `migrateDeadGenieOtel` seam (`codex-config.ts`). Delete: the Codex fallback classification/retirement API, `inspectManagedSkillTree` payload verification, `CANONICAL_GENIE_SKILL_NAMES`, the `agent-sync.js` import block at `:25-39`, and any symbol whose only consumer is a test that dies with it. **Inherited from Group 3 deliverable 5 (transferred 2026-08-31, G3 fix loop 1) — delete the Codex role-agent transaction engine by name, not by inference:** `CODEX_AGENT_INVENTORY_NAME` / `CODEX_AGENT_INVENTORY_OWNER` / `CODEX_AGENT_INVENTORY_MODE`, the `CODEX_AGENT_*` transaction prefixes, the inventory writer and its two call sites, `installCodexAgents`, `convergeCodexPlugin`, `convergeCodexPluginOnly`, and `installCodexIntegration`, together with the `runtime-integrations.test.ts` describes that are their only remaining consumers. They are already unreachable in production on the G3 head (`install.ts`'s `claudeOnlyScope`; `update-integrations.ts` returns `[]` for Codex), so this is a pure removal with no behavior change — and the "every remaining export is named with its consumer" criterion below is what proves it happened.
...
**Acceptance Criteria:**
- [ ] `git grep -rn "agent-sync" -- src scripts plugins package.json` returns **no importer and no stale prose**; every residual hit falls into exactly one of three named classes, each hit classified in the PR, and any fourth class is a regression — **amended 2026-08-31 (G5 fix loop 1), was: "returns nothing, prose included"** (same shape as G3 AC 1 and G4's `src/hooks` AC, amended in their fix loops for the same reason: the token is also the vocabulary of the wire contracts and of the negative guards that prove the deletion, so a literally-empty grep would require breaking a published contract or deleting the proofs). **(i) Wire contract — `genie-agent-sync`** (10 hits): the `managedBy` value a plugin-era Genie wrote into `.genie-sync.json` on real hosts, read by `legacy-integration-retirement.ts:1408` to decide what it may delete, plus the fixtures that must match it byte-for-byte; renaming it would orphan every managed asset already on disk. **(ii) Wire contract — `genie-agent-sync-migration-v1`** (14 hits): the HTML-comment idempotency marker inside **already-published** GitHub release bodies that `scripts/reconcile-release-note.sh:35-36` matches so the one-time migration note is appended at most once; renaming it re-appends that note to every existing release. **(iii) Negative guards** (2 hits, `src/genie-commands/__tests__/update.test.ts:2320,2322`): the assertion — required by AC 3 — that `.last-agent-sync` is absent from `update.ts`. Prose mentions are still a regression; all thirteen sites deliverable 4 names must be gone.
...
### Execution review — Group 3 — 2026-08-31 — FIX-FIRST (round 1), fix loop 1 applied

- Engineer/reviewer: wave-2 agents (reviewer ≠ engineer), session 17ecb3d2. PR #2879 on `wish/skills-everywhere-b-g3`, base commit `6859be5cd`.
- Round 1 returned FIX-FIRST with six findings — **zero code defects among them**: four MEDIUM and two LOW, all "the head is right, the wish text or the ownership record is not". Disposition below; the wish text above carries the amendments inline, each marked `Amended 2026-08-31 (G3 fix loop 1)`.

| # | Finding | Disposition |
|---|---------|-------------|
| 1 | `--publish-local-delivery` terminal moved from exit 2 / `activation-pending` to exit 0 / `delivery-verified` | **Wish amended** (G3 AC 2). Contract change, not a defect: with the activation store deleted there is no record to publish and no step to defer; `verifyDownloadedDeliveryEvidence` still runs in full, and `install.sh:65`'s trailer is proven unreachable. The `release-publish.yml` narrative at `:1016-1034` was reworded to match. |
| 2 | `codex-native-dogfood` guaranteed red on this branch | **Recorded as a release freeze** (callout under G3's acceptance criteria; G6 D7 amended to name the closure). The harness's deletion is G6's by plan, so the red window is a sequencing consequence, not an execution defect: no release may be cut from any head carrying G3 without G6. |
| 3 | D5's Codex role-agent transaction engine still in `runtime-integrations.ts` | **Transferred to G5 D3 by name** (`CODEX_AGENT_INVENTORY_*`, the transaction prefixes, the inventory writer, `installCodexAgents`, `convergeCodexPlugin`, `convergeCodexPluginOnly`, `installCodexIntegration`). Verified dead in production on this head — `install.ts`'s `claudeOnlyScope`, and `update-integrations.ts` returns `[]` for Codex. |
| 4 | D7(e)'s `agent-sync.js` import block still in `uninstall.ts` | **Transferred to G5 D2** with the re-proof obligation stated; (a)–(d) landed and are covered by the record-driven tests. |
| 5 | `.agents` delisting half-done in `update.ts` | **Fixed in code** — and it was hiding a release-breaking defect the review did not reach (below). |
| 6 | The headline grep does not return nothing; `integrationSummary` removed rather than narrowed | **Wish amended** (G3 AC 1 and AC "surviving doctor checks"). The four residual grep hits are classified into exactly two non-importer classes — negative guards and the `'genie-codex-activation-tree-v1\0'` digest domain separator, a wire contract that must stay byte-identical on both sides. |
...
- Independent final-gate reviewer (opus/max effort, no group context), session 17ecb3d2, over the assembled head `fd8a9b32d`. Every wish-level criterion independently re-verified and holding in its ratified form: C4 = 37,454 (−22,735, baseline re-derived per-blob), C5-lite, C7 (six `publish.needs` edges + guards, empty musl diff), C8 (classified form), C9, C11's runnable legs, C-G (102 hits, three classes, no fourth), C-A. All three orchestrator ratifications judged sound, with the amended `--sync-only` contract traced into the code. Cross-group seams probed: 301 relative import specifiers resolve with zero unresolved (including `scripts/`/`tests/`, which tsconfig never typechecks), every workflow-referenced script exists, the four payload-member lists are mutually consistent, `install.sh`'s Codex-activation branch proven dead, every registered CLI flag live. 734 targeted tests pass; PR #2878 head green.
- **One MEDIUM (the sole blocker): the repo's tracked `.claude/settings.json` registered the deleted `genie hook dispatch` on four lifecycle events** (PreToolUse `*`, PostToolUse, UserPromptSubmit, Stop) — the H1 class; `.claude/` was in no group's sweep path. Does not ship (not in the payload lists) and exits 1 not 2, but fires on four high-frequency events for any contributor on a post-Wish-B binary. **Fix applied by the orchestrator in this commit:** the four `hook dispatch` entries deleted; the repo-local `Bash` → `.claude/hooks/git-safety.sh` entry kept. Sweep-path lesson recorded for Wish C: add `.claude/` to the importer-sweep path list.
- LOW handoffs to Wish C (recorded, not blocking): `install.sh`'s dead Codex-activation branch (`:65`, `:840-852`, `:910`) and four stale `agent-sync.ts` cross-references (`:201,:212,:239,:277`); C5-lite worklist additions `skills/README.md:47-48`, `:50-55` and `skills/genie-hacks/references/catalog.md:113`; `src/genie.ts`'s uninstall command description still advertising plugins/marker-owned agents/hooks.

### Execution review — H1 fix wave — 2026-09-01T04:15:00Z — SHIP (round 1)

- Engineer/reviewer: ultracode H1 agents (opus/high, reviewer ≠ engineer), session 17ecb3d2. PR #2885, commit `ef440715e`, merged to `wish/skills-everywhere-b` at `c70672155`. Disposition A executed as ratified: the Claude install-convergence half is deleted (`runtime-integrations.ts` 2,088 → 1,364; `update-integrations.ts` deleted whole; `install.ts` integration-results block gone), all 14 remaining exports consumer-named, keep-set intact. Before/after fixture transcripts prove `genie install --integrations claude|all|auto` all exit 0 where claude/all previously aborted with a stack trace. Full CI gate green on the exact head (run 33460105844). `src/` non-test count: **37,454**.
- SHIP round 1 with three LOW residuals (two false consumer attributions in the PR's export table, a stale sweep paste, and the ledger-wording action discharged by the ratification below).
- **Orchestrator ratification (2026-09-01, session 17ecb3d2):** the G5 `--sync-only` ledger wording is AMENDED — with the Claude payload no longer shipping, the refresh leg is structurally gone, so `--sync-only` = skills-channel convergence → plugin-era retirement, still identical to a plain `genie update`'s convergence on the same head (proven end-to-end in the H1 PR: skills channel → `nothing to retire`, exit 0). The ratified contract's substance — "--sync-only performs the ordinary tracked manual convergence, never touching the binary" — is unchanged.

### Execution review — Group 7 (proof) — 2026-09-01T03:00:00Z — SHIP (round 2) + H1 disposition

- QA/reviewer: ultracode wave-6 agents (opus/high, reviewer ≠ engineer), session 17ecb3d2. PR #2884, commits `6a011f62b` `d740a4aca` + fix `ed32706a4`, merged to `wish/skills-everywhere-b` at `539a6c4ac`. Evidence file: `qa/20260901.md` (1,029 lines; every criterion carries a pasted command + output).
- Headline measurements: **C4 = 38,308 vs baseline 60,189 (−21,881, 97 → 71 files, 1,881 under target)**, baseline re-derived two independent ways at `18b85341b`. C-R4 proven on a real fresh-host cycle from the candidate tarball: 57 agent dirs recorded and measured, collision snapshotted with original bytes preserved, uninstall removed 1,425 recorded dirs, zero genie residue, foreign skills byte-identical. C5-lite worklist handed to Wish C as 8 file:line items (adding `skills/README.md:40-41`, unnamed by the plan). C7/C9/C11/C-A met; C8 met with one classified comment-only residual hunk; C-G discharged by three-class classification. Full gate green in CI on the measured head (runs 33455733167, 33456983012).
- **HIGH finding H1 (blocks the wish merge, not the evidence PR):** `genie install --integrations claude|all` hard-fails on any host with the claude CLI — G4 deleted root `.claude-plugin/marketplace.json` while G5 left `installRuntimeIntegrations`' Claude marketplace registration reachable from `install.ts`. Wish-introduced, reproduced verbatim, root-caused (the G5 keep-list excludes those symbols; the export criterion checked consumer-existence, not runtime reachability).
- **Orchestrator disposition (2026-09-01, session 17ecb3d2): execute disposition A** — delete the Claude install-convergence half (it completes G5 deliverable 3's approved keep-list), preserving the retirement keep-set and the substance of the ratified `--sync-only` contract; disposition B (the manifest-absent guard) is the recorded fallback only if A breaks a ratified contract. Dispatched as fix wave `wish/skills-everywhere-b-h1` under the standard engineer/reviewer protocol; its review lands below.
- LOW residuals recorded: C-G class-(i) file count off by one; C8/C-G literal wording discharged by classification without a wish text amendment (this ledger block is that amendment: **C8 and C-G are RATIFIED as met in their classified forms** — the comment-only orca-bundle-parity hunk stays, since reverting it would restore a dangling reference to the G4-deleted `hook-bundle-parity.ts`).

### Execution review — Group 6 — 2026-09-01T01:50:00Z — SHIP (round 1)

- Engineer/reviewer: ultracode wave-5 agents (opus/high, reviewer ≠ engineer), session 17ecb3d2. PR #2883 (19 files, +256/−4,482), commits `652987170` `3052dc5f1` `9897597f9`, merged to `wish/skills-everywhere-b` at `9772243f4`. SHIP on round 1 with two LOW residuals (C-G's literal "only Orca-owned lines" vs the classified three-class residual set; the six-edge count asserted by line regex rather than parsed YAML).
- Deliverables 1, 2, 3-code and 6 had been legitimately pulled forward by G3/G4/G5 — verified and recorded with pasted evidence rather than redone; G6 added the two stale prose counts no test caught, the strictly-parsed `version.ts --check` (argv resolved before any mutation), the `build.js`/`sync.js` deletion with the generator-equality test rewritten against the committed manifest shape, and the release-publish.yml surgery: exactly the two dogfood jobs and exactly one `publish.needs` edge removed, six edges + if-guards byte-intact, `candidate-dogfood-matrix.ts` explicitly KEPT with both consuming jobs named and pinned by a new test. musl surfaces byte-unchanged vs origin/dev (C7 diff empty).
- Recorded deviations: `prepare-delivery-evidence`'s dead `dogfood_matrix` output/variable removed (only consumers were the two deleted jobs; the projected JSON artifact untouched); `release-guard.sh` proven via `release-guard.test.ts` (39 pass) since both invocation forms fail-closed outside Actions; version.yml's `workflow_run` inertness on dev recorded with version-format/version-ci-staging as evidence. Full Quality Gate passed in CI on the pre-merge head (run 33454060166).

### Execution review — Group 5 — 2026-09-01T00:45:00Z — SHIP (round 2)

- Engineer/reviewer: ultracode wave-4 agents (opus/high, reviewer ≠ engineer), session 17ecb3d2. PR #2882, commits `d572f9e0b` `699a48bbd` `6df3c67d6` + fix `93744f64f` `fe15723dc`, merged to `wish/skills-everywhere-b` at `54f42caf1`. `src/` non-test count after merge: **38,308 — the C4 ≤40,189 target is met at this head**, one group early.
- Round 1 FIX-FIRST; round 2 SHIP with one MEDIUM and four LOW residuals. The MEDIUM was the pending ratification below; the LOWs (install.sh stale agent-sync cross-references, AC-5's unrunnable literal `release-guard.sh` invocation with `release-guard.test.ts` 39-pass as the substituted proof, no uninstall-seam test for Codex role-agent removal, two cosmetic prose leftovers) are recorded, none blocking.
- **Orchestrator ratification (2026-09-01, session 17ecb3d2):** the G5 fix-loop-1 wish amendment in `fe15723dc` is RATIFIED — (a) `--sync-only` widening to the ordinary tracked manual convergence (skills channel → refresh → backup-first idempotent retirement, still branching before every fetch/download/swap) is accepted over the deprecation-no-op and retirement-narrowed alternatives for exactly the recorded reasons; (b) the uninstall journal v3 → v4 bump with v3 joining the authenticate-then-discard-then-re-record legacy lane in both the reader and the discard guard; (c) the AC-1 three-named-class grep amendment (two wire contracts + the AC-3 negative guards), same shape G3/G4 carry.
- D5 importer sweep: 351 hits/42 files classified pre-deletion; post-deletion 49 residuals in the three ratified classes. `runtime-integrations.ts` 4,339 → 2,087 with all 23 remaining exports consumer-named. Both G3 inherited debts settled (role-agent transaction engine deleted by name; uninstall D7(e) done, −890-line managed-asset subsystem).

### Execution review — Group 4 — 2026-08-31T22:20:00Z — SHIP (round 2)

- Engineer/reviewer: ultracode wave-3 agents (opus/high, reviewer ≠ engineer), session 17ecb3d2. PR #2881 (220 files, +269/−21,534), commits `e250b9463` `18a071e21` + fix `fa41207b8` `ff44095a6`, merged to `wish/skills-everywhere-b` at `f385364aa`. `src/` non-test count after merge: 49,485.
- Round 1 FIX-FIRST; round 2 SHIP with four LOW residuals recorded, none blocking (validate-wish usage text naming the deleted .cjs, D7 byte-diff satisfied at the pre-deletion snapshot, stale PR line refs, two prose sites describing the deleted hook runtime).
- `check` is now exactly the eight steps (`check:fast` seven) the wish specifies; the deletions and package.json surgery landed in one commit. D10 importer sweep: 160 hits classified pre-deletion (surfacing three unnamed test files, deleted with their subjects); post-deletion re-run leaves four prose/data hits.
- Recorded deviations: fresh-install-smoke's plugin-layout half and `build-binary.sh`'s `--plugin-root` arguments pulled forward from G6 (forced — the payload they read no longer ships); the root `.claude-plugin` tarball payload member delisted from `INSTALL_PAYLOAD_MEMBERS`/`AUX_LAYOUT_DIRS`/five toolchain lists in the same commit (unnamed in the wish; same class as G3's `.agents`), taking `version.yml` from eight version files to five with tests re-pointed; hook-orphaned doctor/omni symbols deleted rather than left dead-reachable (knip-verified). G3's role-agent-engine note re-verified: still legitimately reachable via `install.ts`/`update-integrations.ts` convergence — G5 owns it as planned.
- AC-1 grep is non-empty by necessity: every residual classified in the PR into negative guards/banned-token lists and retirement surfaces for host assets a previous Genie wrote (same shape as G3's amended AC).

### Execution review — Group 3 — 2026-08-31T21:05:00Z — SHIP (round 2)

- Engineer/reviewer: ultracode wave-2 agents (opus/high, reviewer ≠ engineer), session 17ecb3d2. PR #2879 (110 files, +1,323/−27,788), commits `5109bf0fb` `d53c54872` `9febace07` `83abf3f20` `6859be5cd` + fix `6b3ecde26` `add9e4a8a` `7a73e740e`, merged to `wish/skills-everywhere-b` at `172542843`. `src/` non-test count after merge: 52,692 (baseline 60,189).
- Round 1 FIX-FIRST; round 2 SHIP with five LOW residuals recorded, none blocking (PR grep-paste freshness, D11 sweep summarized vs raw, no test on the new uninstall retirement step, G6-pull-forwards recorded in PR only, cosmetic stale Codex prose).
- D11 importer sweep: 273 hits/63 files classified pre-deletion; post-deletion re-run leaves 4 non-importer hits (2× the `genie-codex-activation-tree-v1` digest domain separator wire contract, 2× `setup.test.ts` negative-assertion list). D13 table reproduced; `codex-project-route-migration.test.ts` byte-unchanged.
- Recorded deviations riding to later groups: the Codex role-agent transaction engine remains reachable via `convergeCodexPluginOnly`/`convergeCodexPlugin` (G4/G5 own those convergence paths); uninstall D7(e) — dropping the agent-sync import block and flat-agent transaction machinery — deferred to G5 where `agent-sync.ts`'s deletion forces it mechanically; `src/fixtures/codex-*.json` partially retained (trusted-root RENAMED to `delivery-public-good-trusted-root.json` for the surviving Sigstore verifier; fallback/role-agent fixtures still feed G5-owned consumers).
- Toolchain proof run in-group: `build-binary.sh --platform linux-x64-glibc` green (35 MB tarball, double verify + 2× fresh-install-smoke), `release-payload-version.ts --verify-source .` OK.

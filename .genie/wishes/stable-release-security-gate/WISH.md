...
- [x] Final `bun run check` and full `bun test` are green on the exact rebased PR tree (1,957 tests / 6,945 assertions; zero failures).
- [x] Independent review of the final exact PR tree returns SHIP. *(Overall and release-chain reviews of `8870c57cf7c054eb695986359733602bc7d3d47e` returned SHIP; the transaction specialist's VERSION-binding follow-up was fixed and independently re-reviewed SHIP before the final evidence amendment.)*

...
- `genie uninstall`: delete the recorded inventory skill dirs from the recorded agent dirs, then the record (deterministic; no dependency on the experimental lockfile) (amended 2026-08-31 from the PR #2866 promotion review: removal is now digest-verified — the record carries a per-directory content digest and only a recomputed match is deleted, while legacy records without digests (5.260830.x) and user-modified or foreign same-name dirs are preserved and reported; the record itself is deleted only after a fully clean sweep so an incomplete removal stays retryable).
...
### Closure correction, part 2 — 2026-08-31 — owned by Wish B Group 1
- **Owner of the fix, and its shape.** `skills-everywhere-b` Group 1 (branch `wish/skills-everywhere-b-g1`) implements the correction above: `buildSkillsAddArgv` now emits `npx -y skills@1.5.23 add <GENIE_HOME>/skills --all --copy -g` — the signed tarball's own delivered tree, the only source genuinely pinned to the running binary — and the record carries `source: 'local:<abs root>'` beside `ref: v<VERSION>`. `git grep -n "automagik-dev/genie@" -- src` returns nothing on that branch. The same group replaces the four-row `agentDirs` table with a bounded `$HOME` discovery scan, so the `into N agent dir(s)` line reports what the CLI actually wrote (G7 HIGH-1), and snapshots/backs up any foreign same-named skill directory before `--copy` overwrites it.
- **Upstream follow-up (recorded, not filed).** `vercel-labs/skills`: `skills add <repo>@<ref>` silently ignores the `@<ref>` suffix and serves the repository's default branch instead of the requested ref (`dist/cli.mjs` binds the third regex capture to `skillFilter`, not to a ref; only a `#fragment` is treated as a ref). Reproduced on 1.5.23 three ways on 2026-08-31 — a feature-branch head, a bare 40-hex SHA, and tag `v5.260712.1` (whose tree carries `pm`/`wizard` and no `quick`) all returned the identical 22 names *including* `quick`, i.e. `main`'s tree; a local path source resolves correctly. Whether to file the issue is the user's call — no genie behaviour depends on it any more.
- **The public documented path still serves `main`, and that is accepted.** `docs/installation.mdx` documents `npx skills add automagik-dev/genie` for operators who want the skills without genie. That command resolves the repository's **default branch**, not a release tag — acceptable for a public, unversioned convenience path, and unchanged by this group. Re-wording the page to say so explicitly belongs to Wish C (its docs worklist).

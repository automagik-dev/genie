...
- **`dsh-genie-board`** stays `IN_PROGRESS` with a dated parked note. **`orca-plugin-genie`** was superseded on 2026-09-26, when the owner retired the Orca integration (`retire-orca-integration`, issue #3064). **`observe-skill-bundle`** and **`dsh-workflow-fork`** stay `APPROVED` and have never been executed. None of them is being worked on.

...
- [WISH: dsh-workflow-fork](wishes/dsh-workflow-fork/WISH.md) — **DRAFT** (re-opened from FIX-FIRST 2026-09-19): fork a genie-owned workflow runner for DSH bodies. Nothing has executed — `automagik-dev/dsh_workflow` does not exist, all five cards are unclaimed — but Group 0's seam comparison and spike now exist on branch `wish/dsh-workflow-fork-group0` (PR pending review): a loader that splits `export const meta` off the body and drops `effort` ran `.claude/workflows/council.js` to completion on host `dsh 0.1.5-rc.2` (`6 agents`, `decision: gather-evidence`), and the comparison chooses the loader over the QuickJS fork, leaving the fork as the fallback for the triggers it names — and the wish's own 2026-09-15 council recorded that DSH 0.1.x already ships a first-party `ctx.workflowEngine` that may make the fork unnecessary. Group 0 decides the route before anything downstream starts

...
- [WISH: dsh-workflow-fork](wishes/dsh-workflow-fork/WISH.md) — **APPROVED** (orchestrator, 2026-09-20), re-planned 2026-09-19 against the route Group 0 chose: run the catalog on DSH's first-party `ctx.workflowEngine` with a loader, and do **not** fork `omdsh-dev/dsh_workflow` (`automagik-dev/dsh_workflow` is not created, and Groups 1–3 are delivered in genie, moot, or deferred behind a named trigger). Group 0's evidence is accepted (#3008 merged): a loader that splits `export const meta` off the body and drops `effort` from `agent()` options ran `.claude/workflows/council.js` to completion on host `dsh 0.1.5-rc.2` (`6 agents`, `decision: gather-evidence`), with 35 option removals across 7 of 9 scripts and every `effort`-named schema field preserved. The loader itself is `plugins/dsh-workflow-loader` (#3011 merged; payload wiring and a boot smoke in #3015), and its smoke, on #3015's branch (still in review), proved a real Host boots with the row active after catching two loader defects. Residual work this wish still owns: the cross-body engine comparator, the recorded `council` integration run, and the deferred adapters. Awaiting the re-review that gates the residual work; the cards stay unclaimed

## Poured

- [orca-plugin-genie — DESIGN](brainstorms/orca-plugin-genie/DESIGN.md) → [WISH](wishes/orca-plugin-genie/WISH.md) — **IN_PROGRESS**: the Orca plugin as a real integration on Orca's native mechanisms — eight genie verbs in the palette and keybindings, the lifecycle mirrored one-way onto the existing board cards (`genie orca mirror`), human decisions as Orca gates, result notifications; the adapter allowlist amended (worktree show/set, terminal list, `worker-start --spec`) and repaired for Orca 1.4.205; RF4/RF5 designed only. Wave 1 on `namastex888/orca-plugin-genie`, merge-ready PR [#3019](https://github.com/automagik-dev/genie/pull/3019) against dev

...
| **Rev.** | **3.3** (2026-08-25) — rev. 3 reescrita sobre a rev. 2 após council de 2 rodadas × 5 lentes; **.1** = loop de correção dos 14 achados da review da rev. 3; **.2** = loop dos 6 major + 7 minor da segunda review; **.3** = loop dos 3 major + 8 minor da terceira |
| **WRS** | 100/100 |
| **Council** | [COUNCIL.md](COUNCIL.md) · dossiê em [council/](council/) |
| **Design review** | rev. 2: FIX-FIRST 2× (`581a20fa…`). Rev. 3: **FIX-FIRST** (`c4fd4cbf…`) — [relatório](reviews/design-review-rev3-20260825.md), 14 achados. Rev. 3.1: **FIX-FIRST** (`213dc675…`). Rev. 3.2: **FIX-FIRST** (`2fd44d34…`) — 0 critical, 3 major, [relatório](reviews/design-review-rev32-20260825.md). **Rev. 3.3 aguarda a quarta review.** |
...
- O **tier global** (`genieHome()/config.json`) ganha `execution.mode` no `GenieConfigSchema` (`src/types/genie-config.ts:205`), que é `z.object` puro sem `.strict()`: a chave é aditiva e hoje é silenciosamente *stripped*, nunca hard-fail. Mesma forma de `codex`/`otel`/`omni`. O tier de repo não passa por Zod — `.genie/mode` é um enum de uma linha.
...
- O gate vive **dentro de `openDb()`** (`src/lib/v5/genie-db.ts:404`), antes do `openSqlite` — não em cada verbo. É o único ponto que cobre de uma vez os call-sites restantes — 29 hoje, **26** depois que o `mcp` sai (`mcp-tools.ts:203,205,367`), dos quais só três passam `path` explícito (`doctor.ts:287`, `context.ts:218`, `v5-board.ts:261`) e um passa `cwd` (`ui-bridge.ts:242`, que sobrevive ao PR 1 e só morre no PR 2 — no intervalo ele lança `ModeRefusalError` sem tradução, o que é aceitável porque `ui-bridge` já está condenado e não é gate) e os git hooks. **O gate resolve o modo a partir do repo dono do `dbPath`**, nunca do `process.cwd()` — mesma âncora `git-common-dir` que resolveu o caminho do banco, e é a única forma de a asserção forte (`.genie/genie.db` **não existe**) ser verdadeira. `openDb` é `CREATE TABLE IF NOT EXISTS`: uma abertura acidental deixaria `count(*) FROM tasks = 0` e passaria em falso.
...
- **Recusa é um erro tipado; quem traduz são quatro pontos nomeados.** `openDb()` recebe só `{path, cwd}` (`src/lib/v5/genie-db.ts:404`) — não sabe qual verbo o chamou, logo **não** escolhe exit code. Ele lança `ModeRefusalError`. A tradução acontece em exatamente quatro lugares, e nenhum outro:
  ...
  3. **`genie doctor`** (`checkDatabase`, `src/genie-commands/doctor.ts:275-306`): hoje qualquer throw do `openDb` vira `{status:'fail'}`, e `ok: failed.length === 0` (`:2304`) + `process.exitCode` (`:2330`) deixariam o **doctor vermelho em todo repo orca convertido de v5** — porque o critério (a) manda o `genie.db` continuar existindo lá, e `checkDatabase` só pula quando o arquivo não existe (`:277`). Passa a capturar `ModeRefusalError` e emitir `skip` ("board congelado em modo orca"), nunca `fail`.
  4. **`openWishDb`** (`src/term-commands/context.ts:213-224`): re-lança `ModeRefusalError` em vez de convertê-lo em `failClosed('unreadable-db')`. Com o degrade do item seguinte esse ponto é **defesa em profundidade** — `--wish` em orca implica `--plan` e nunca chega a abrir o banco —, mas a tradução fica escrita porque o mascaramento é real (`contextCommand`, `:416-424`, converteria qualquer throw em `{"error":"internal"}`).
...
- **`genie mcp` sai inteiro** (Felipe, 2026-08-25: "só tava testando") — `src/lib/v5/{mcp-server,mcp-tools}.ts`, o verbo `mcp`, e **as duas rotas** que `genie init` escreve hoje: a entrada `genie` em `.mcp.json` **e** a rota marker-owned em `.codex/config.toml` (`src/term-commands/init.ts:206` → `registerMcpConfigs(root, { codexEntry: genieFacadeMcpEntry(), forceCodexFallback: true })` → `src/lib/codex-project-mcp.ts`). Sai junto o check de rota de projeto do doctor — `codexProjectRouteCheck` (`src/genie-commands/doctor.ts:507-564`) e o import de `codex-project-mcp` em `:67`. **Não** `hasDuplicateMcpGenieKeys` (`:77`, usado em `:1607` dentro de `checkHermesMcp`): esse é o check do `~/.hermes/config.yaml` do agent-sync e sai com a máquina Hermes, não com o MCP — senão, entre a deleção do verbo e a deleção da máquina Codex, o `init` reconcilia uma rota para um verbo inexistente e o `doctor` a audita. Fecha por deleção o buraco das 17 tools `genie_*` e remove 3 call-sites de `openDb`.
- **Onde fica a borda.** A rev. 3 inicial dizia que "a borda já existe" no `program.hook('preAction')` de `installWorkspaceCheck` — **falso e verificado**: `WORKSPACE_EXEMPT` (`src/lib/interactivity.ts:45-92`) isenta `task`, `board`, `context`, `idea`, `init`, `doctor` e `install`, exatamente os verbos que o modo precisa gatear, e `commandRequiresWorkspace` (`:109`) retorna antes de qualquer coisa. A borda mecânica do modo é **o próprio `openDb()`** (estado) mais um `preAction` **novo e não-isento** cuja única função é transformar a recusa numa mensagem legível para os verbos de `ORCA_FORBIDDEN`. Duas camadas, uma verdade: as duas chamam o mesmo resolvedor.
...
- O router `genie` perde a tabela "Operational Command Mapping" (`skills/genie/SKILL.md:68-92`; "board" aparece 5× na tabela e 8× no arquivo) e passa a delegar por modo; State Detection: `APPROVED → genie-orca-work` em modo orca.
...
| 20 | Validador com fixture por modo (`wish-template.orca.md`, `--mode orca`) para a **estrutura**; classic ignora `## Dispatch plan`. As regras de **conteúdo** (13* e o enum de `Tracker`) não cabem em fixture e são código no validador, ativado por `--mode orca` |
...
| # | Risco | Sev. | Mitigação |
|---|---|---|---|
| 1 | Modo orca sem base de worktree pinada | High | Decisão 6 (`--plan` + header `Base`) **mais** a Decisão 13*, que faz o validador reprovar a wish sem SHA de 40 hex — sem o enforcement a mitigação seria só disciplina, porque sem banco a base é recomputada a cada chamada |
...
- [ ] **(c) Clássico intocado, com duas exceções nomeadas.** Golden de `genie --help` idêntico **exceto as linhas `mcp` e `omni`** (ambos deletados por decisão explícita); `genie task --help` e `genie board --json` byte-idênticos; o schema de `task export` idêntico **exceto a chave `hire_roster`**, que sai com a tabela no PR 2 — ela é chave de primeiro nível de `ExportState` (`src/lib/v5/task-state.ts:1610,1653`), está no `EXPECTED_SCHEMA` (`src/lib/v5/genie-db.ts:416`), é lida por `importState` (`task-state.ts:1804,1879,1885`) e aparece como `hire_roster: []` no `roadmap.json` commitado (`src/lib/v5/roadmap-sync.ts:60`); o bump de `user_version` é a prova da migração, e o golden do `roadmap.json` é atualizado no mesmo PR; `tests/e2e/v5-lifecycle.sh` verde, com o **único** diff sendo a remoção do bloco 9b inteiro (`:367-442` — inclusive `nats-connection-count-zero-on-load` e `nats-import-is-dynamic-only`, `:425-442`, que leem `src/lib/omni-runner.ts` e morreriam com `omni-runner source not found` assim que o PR 5 deletar o arquivo) — a asserção `help-lists-omni-task-board` (`:400-404`, que hoje exige que `--help` liste `omni`) está **dentro** desse bloco e sai junto, não é um diff separado; sem env e sem `.genie/mode`, `resolveExecutionMode()` retorna `classic`.
...
- [ ] **(e) Nada a reconciliar.** O critério é **comportamental, não arquitetural**: não existe caminho de código que escreva estado de board em modo orca. A prova estrutural é uma só e é estável — **toda escrita passa por `openDb`** (todos os demais opens diretos em `src/` são `readonly: true`), e `openDb` recusa em orca. A prova executável é a suíte de modo: para cada verbo de `ORCA_FORBIDDEN`, mais `task sync`, `context --wish`, `doctor` e um commit real, nenhum cria ou muta `.genie/genie.db` nem `roadmap.json`.
  > **Nenhuma forma de contagem de módulos serve, e isto é lição paga três vezes:** "interseção vazia" foi falsificada pelo gate morar em `genie-db.ts`; "exatamente dois módulos" quebrou quando `v5-task.ts` passou a traduzir a recusa do `sync`; "exatamente quatro pontos" é falsificada pelo próprio design, que manda `context.ts` ler o modo duas vezes (`mode=` e o degrade de `--wish`) e `init.ts` ler para escolher o conjunto de hooks. Um invariante que enumera importadores quebra no PR seguinte, sempre. E, em vez de afirmar a ausência de uma feature que nunca existiu — nada em `src/`/`scripts/` instala git hook —, a asserção é sobre o que existe: num repo orca, um **commit real** não deixa `roadmap.json` nascer, e o `.husky/pre-commit` sai 0 e silencioso.

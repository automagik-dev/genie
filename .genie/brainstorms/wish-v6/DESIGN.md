...
| **Slug** | `wish-v6` |
| **Date** | 2026-09-16 |
| **Revision** | 4 — council `revise` applied ([COUNCIL.md](COUNCIL.md), run `wf_f4a43c6d-b1f`), design review FIX-FIRST findings 1–9 applied, then PR #2930 Codex P1s (hooksPath read-only query; retry never deletes) applied |
...
- `skills/wish/SKILL.md` rewritten as the front door in the council pattern: names the script path and saved name; states what is relayed unchanged and what stays with the operator (merge, SHIPPED, promotion, worktree and branch removal after merge with `git worktree remove <path> && git branch -d wish/<slug>` — `-d`, so an unmerged branch is refused; retry after `missed`/`blocked` is a rerun with the same objective, which adopts the named worktree when it is clean with nothing diverged from the remote and is otherwise `blocked` with the diagnostic above; on `pr-open` the front door names the re-read command, `gh pr checks <n>`); offers a **direct plan entry** that skips admission when the operator already declares a multi-group objective; keeps verbatim the `wish-scaffold-command` fence, the `templates/wish-template.md` name, the design-review verify command and the plan-route phrases `scripts/release-docs.test.ts` and `scripts/fresh-install-smoke.ts` pin; states that the by-hand section is the only working path on installed hosts until #2916 lands, and makes it complete enough to deliver a PR; names no client tool and carries no repo-local command strings or darwin test names (#2917, `AGENTS.md:33`). Frontmatter `mutates: repo`.
...
- `.claude/hooks/git-safety.sh` extended to exit 2 on `gh pr merge`, `gh api -X (PUT|POST|PATCH|DELETE)`, `HUSKY=0`, `core.hooksPath` in override or mutation form only (`-c core.hooksPath=…` and `git config … core.hooksPath <value>`; the read-only `git config --get core.hooksPath` the gate runs stays allowed), and refspecs ending in `:main`/`:dev`/`:master`; its prefilter today matches only commands containing `git`, so it widens to `gh ` as well.
...
| 6 | Orphaned worktrees and branches | Low | Result names the worktree; a rerun adopts a clean worktree whose remote is absent or an ancestor of its head, and is otherwise `blocked` with a diagnostic that shows unpushed commits; the workflow never deletes a worktree or branch, and removal after merge is the operator's with a non-forcing `git branch -d`. |
...
- **Verdict:** SHIP
- **Reviewed content SHA-256:** `25af2eae5ba5483df231e2c37fa5123e3583b86cf544233d5c05d3b7b9597f18`
- **Reviewer:** design-review@session-8e0382e3
- **Reviewed at:** 2026-09-16T20:34:55.000Z

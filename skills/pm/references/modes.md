...
Spawn one decision-maker subagent via the **Agent tool** at run start. Route ship/no-ship and prioritization judgment calls to it via **SendMessage**; it emulates the human within Authority Boundaries. Anything past a boundary still goes to the real human.
...
Provide your own persona prompt in the Agent-tool brief instead. A persona defines:

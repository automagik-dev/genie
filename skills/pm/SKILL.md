...
| Validate | `review` | Gate every group; FIX-FIRST → `fix` (resolved budget, default 2, capped at 5 without a recorded human decision) |
...
| Review returns FIX-FIRST | Diagnose first; simplify an overdesigned plan, otherwise `fix` (resolved budget, default 2, capped at 5 without a recorded human decision, then escalate) |

...
The v5 "lightweight body" cutover re-plumbs skills onto three primitives — **documents in git**, **operational state via the `genie v5` CLI** (zero-daemon SQLite), and **dispatch via Claude Code native teams** (the Agent tool). This section is the keep/drop decision for every current skill dir; it is a **list decision, not a set of rewrites** — only the core four are rewritten in this group (v5-foundation Group 3). The rest are ported in their own later groups.

Decision legend:

- **Keep — core (rewritten here):** rewritten free of the v4 runtime in this group.
- **Keep — portable now:** no intrinsic v4-runtime coupling; the only change needed is re-pointing dispatch to the Agent tool and/or state to `genie v5` — mechanical, no design work.
...
|-------|----------|-----------|
| `brainstorm` | Keep — core (rewritten here) | Ideation → DESIGN.md. WRS scoring and crystallize are pure methodology; only the tracking-task call moved to `genie v5 task`, artifacts stay in `.genie/`. |
| `wish` | Keep — core (rewritten here) | DESIGN.md → WISH.md with groups + DAG. Scaffold is now a `cp` of `templates/wish-template.md`; per-group tasks via `genie v5 task`; lint via `bun run wishes:lint`. |
| `work` | Keep — core (rewritten here) | Wave dispatch + fix loops + validation. Dispatch is now the Agent tool (native team), state via `genie v5 task checkout/done`, completion by notification (no polling). |
...
| `genie` | Keep — portable now | Natural-language router into the other skills. Routing logic is runtime-agnostic; any command hand-offs re-point to the `genie v5` namespace during its own port. |
| `wizard` | Keep — portable now | Onboarding flow (scaffold → first wish → execute). No intrinsic PG dependency — its steps map onto git docs + `genie v5` + native-team dispatch. |
...
| `council` | Keep — portable now | Convenes multiple AI agents for deliberation — a natural fit for native teams (Agent tool spawns members, SendMessage for cross-talk). Dispatch is the only thing to re-plumb. |
...
| `pm` | Keep — port deferred | PM playbook (triage/prioritize/assign/track/report). Copilot and pair modes map onto `genie v5 board`/`task` + native-team dispatch and are portable; **autopilot** mode depends on the same background-execution substrate as `dream` (below), so the full port waits on it. |

...
3. On FIX-FIRST: diagnose first; return an overdesigned plan to wish/design review, otherwise dispatch `fix` for valid gaps using the resolved fix-loop budget (default 2 unless a higher-priority instruction overrides it, capped at 5 without a recorded human decision). On another architectural issue: escalate in the report, no fix attempt.

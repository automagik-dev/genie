...
Checked 2026-09-15. OpenAI’s latest-model URL may change; refresh the existing guide deliberately while keeping the two provider switches stable.
...
5. Report the output path, provider/default, meaningful changes or assumptions, and official source link. Keep relevant model/effort/API/history settings in the report; a rewrite does not change the runtime configuration.

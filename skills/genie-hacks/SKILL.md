...
3. **Submit** — per `references/contributing.md`: preflight `gh`/`git`, fork + clone `automagik-dev/docs` into the `~/.genie/cache/docs-fork` cache, branch `hack/<slug>`, append under the category heading in `genie/hacks.mdx`, commit `hack: <title>`, open the PR against `dev`.

...
Lead with a one-sentence verdict on architectural health. Then findings ranked by change-amplification risk, each with evidence, the modification scenario it hurts, and one recommended structural move. Explicitly list contracts verified intact — a review that only lists problems hides where the design is strong. Cross-lane handoffs last. In a genie-framework repo, tag severities in `/review` vocabulary (SHIP / FIX-FIRST / BLOCKED) and note which findings warrant a refactor wish via `/wish` rather than opportunistic edits.

...
- **No task row?** (wish predates the state DB, or `.genie/genie.db` unavailable): skip every `genie task` call including report and comment and drive the wave from the WISH.md directly — task tracking is an enhancement, never a blocker. Under `orchestration.mode = orca` Orca is the lifecycle authority and owns the card conversation; local `genie task` writes are refused by design, so post the same moments through the Orca surface instead.

...
2. **Dispatch tracer:** native delegation surface → trace subagent, briefed with the symptoms, relevant context (files, recent changes, environment), and read-only stop conditions (see Dispatch).
...
Trace runs in **isolation**: the subagent is read-only (Read, Bash, Glob, Grep — no Write, no Edit) and must not modify any source file. Spawn via the native delegation surface; follow-ups to a running tracer go through native follow-up messaging.

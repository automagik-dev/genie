...
inject_orchestration_prompt() {
    ...
    local dest="$rules_dir/genie-orchestration.md"

    mkdir -p "$rules_dir"

    cat > "$dest" <<'ORCHESTRATION_HEREDOC'
...
ORCHESTRATION_HEREDOC

    success "Orchestration prompt written to $dest"
}
...
run_install() {
    ...
    # ─── Inject orchestration prompt into ~/.claude/rules/ ───
    inject_orchestration_prompt

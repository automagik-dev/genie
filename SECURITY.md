...
Release-channel authority comes from the repository-hosted `.well-known/latest.json`, `homolog.json`, and `dev.json` manifests. Every dev, homolog, or stable promotion creates a fresh monotonic version and immutable tag from its CI-approved channel source. The exact release becomes public and immutable before the eligible manifest advances; published release bytes and state are never edited. GitHub's release metadata remains a non-authoritative convenience: stable releases are non-prerelease and marked Latest, while dev and homolog releases are prereleases and never Latest. A fixed migration caveat may be appended to human-authored release notes.

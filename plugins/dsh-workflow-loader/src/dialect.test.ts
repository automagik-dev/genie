...
describe('splitMeta', () => {
  test('reads a pure literal without evaluating it', () => {
    const { meta, body } = splitMeta(`export const meta = { name: 'x', description: 'y', phases: [{ title: 'A' }] }\nreturn 1`)
...
describe('stripDeferredOptions', () => {
  const cases: Array<[string, string, number]> = [
    ...
    ['after a spread', `const r = await agent('go', { label: 'a', ...(M ? { model: M } : {}), effort: 'low' })\nreturn r`, 1],
    [
      'in several calls',
    ...
    ['when a schema field is named effort', `const S = { effort: enumOf(['low']) }\nconst r = await agent('go', { label: 'a', schema: S })\nreturn r`, 0],
    ['inside a template-heavy body', 'const q = `a ${fn(1)} b`\nconst r = await agent(q, { effort: "low" })\nreturn r', 1],
    ['beside a regex literal', `const re = /^[a-z]+$/\nconst r = await agent('go', { label: 'a', effort: 'low' })\nreturn r`, 1],
  ]
...
describe('scanUnsupported', () => {
  ...
  test('does not fire on prose or a similar identifier', () => {
    expect(() => scanUnsupported(`// call budget(x) only on Claude Code\nconst budgetable = 1\nreturn budgetable`)).not.toThrow()
  })
})

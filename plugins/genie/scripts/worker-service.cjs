...
`)),t?n(e,{context:t}):n(e,{context:null});return}if(s==="/api/admin/restart"&&c==="POST"){n(e,{success:!0,message:"Worker restarting..."}),setTimeout(()=>{process.exit(0)},100);return}n(e,{error:"Not found",path:s},404)}function S(){try{if((0,i.existsSync)(g)){let o=Number.parseInt((0,i.readFileSync)(g,"utf-8").trim(),10);return process.kill(o,0),!0}}catch{}return!1}function I(){(0,i.writeFileSync)(g,String(process.pid))}var k=process.argv[2];if(k==="start"){if(S()&&(console.log("Worker already running"),process.exit(0)),process.argv[3]!=="--foreground"){let e=(0,p.spawn)(process.argv[0],[process.argv[1],"start","--foreground"],{detached:!0,stdio:"ignore"});e.unref(),console.log(`Worker started (PID: ${e.pid})`),process.exit(0)}let o=(0,T.createServer)((e,a)=>{y(e,a).catch(s=>{console.error("Request error:",s),n(a,{error:"Internal server error"},500)})});o.listen(u,"127.0.0.1",()=>{I(),console.log(`genie worker listening on http://127.0.0.1:${u}`)}),process.on("SIGTERM",()=>{o.close(),process.exit(0)}),process.on("SIGINT",()=>{o.close(),process.exit(0)})}else if(k==="stop"){try{if((0,i.existsSync)(g)){let o=Number.parseInt((0,i.readFileSync)(g,"utf-8").trim(),10);process.kill(o,"SIGTERM"),console.log("Worker stopped")}else console.log("Worker not running")}catch{console.log("Worker not running")}process.exit(0)}else if(k==="status"){if(S()){let o=(0,i.readFileSync)(g,"utf-8").trim();console.log(`Worker running (PID: ${o})`);try{(0,p.execSync)(`curl -s http://127.0.0.1:${u}/health`,{encoding:"utf-8"}),console.log("Health: OK")}catch{console.log("Health: Unable to connect")}}else console.log("Worker not running");process.exit(0)}else if(k==="hook"){if(process.argv[3]==="context")try{let e=(0,p.execSync)(`curl -s http://127.0.0.1:${u}/api/hook/context`,{encoding:"utf-8"}),a=JSON.parse(e);a.context&&console.log(`
<genie-workflow>

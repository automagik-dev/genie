...
- The shared-workspace git-state freeze is operator policy carried in briefs and in this file, enforced by nothing client-side; that half of [#2705](https://github.com/automagik-dev/genie/issues/2705) stands. Merging no longer is: since 2026-09-17 `.claude/hooks/git-safety.sh` refuses `gh pr merge`, an API merge or protected-ref move, and any push aimed at `main`/`master`, in every spelling — so in a Claude Code session an agent merges NOTHING, to `dev` or anywhere else, and reports merge-ready instead. Merging stays the operator's, from their own shell or the web UI. The hook deliberately does NOT guard ordinary pushes to `dev`: that rule remains operator policy with no client-side guard. `main` also stays protected server-side.

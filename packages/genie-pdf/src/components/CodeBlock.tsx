...
export function CodeBlock({ code, language, theme, inline = false }: CodeBlockProps) {
  const isGlass = theme.name === 'glass';
  const isDark = theme.name === 'dark';

  const styles = StyleSheet.create({
    codeBlock: {
      backgroundColor: isGlass ? 'rgba(241, 245, 249, 0.8)' : isDark ? '#1f2937' : theme.colors.codeBg,
      borderRadius: isGlass ? 8 : 4,
      padding: 12,
      marginVertical: 10,
      borderLeftWidth: isGlass ? 4 : 3,
      borderLeftColor: isGlass ? 'rgba(99, 102, 241, 0.4)' : theme.colors.border,
      ...(isGlass && {
        borderWidth: 1,
    ...
    inlineCode: {
      ...
      backgroundColor: isGlass ? 'rgba(99, 102, 241, 0.1)' : theme.colors.codeBg,
      paddingHorizontal: 4,
      paddingVertical: 1,
      borderRadius: 3,
    },

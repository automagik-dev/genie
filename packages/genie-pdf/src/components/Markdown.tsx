...
export function Markdown({ tokens, theme }: MarkdownProps) {
  ...
  const _isExecutive = theme.name === 'executive';
  const _isDark = theme.name === 'dark';
  ...
  const styles = StyleSheet.create({
    ...
    blockquote: {
      ...
      borderLeftColor: isGlass ? 'rgba(99, 102, 241, 0.6)' : theme.colors.border,
      paddingLeft: 14,
      paddingVertical: isGlass ? 10 : 4,
      marginVertical: 12,
      marginLeft: 0,
      backgroundColor: isGlass ? 'rgba(99, 102, 241, 0.06)' : 'transparent',
    ...
    hr: {
      borderBottomWidth: 1,
      borderBottomColor: isGlass ? 'rgba(148, 163, 184, 0.3)' : theme.colors.border,
      marginVertical: 16,
    },
    ...
    return tokens
      .map((token, index) => {
        switch (token.type) {
          ...
          case 'codespan':
            return <CodeBlock key={index} code={(token as Tokens.Codespan).text} theme={theme} inline />;
          case 'link': {
  ...
  const renderToken = (token: Token, index: number): React.ReactNode => {
    switch (token.type) {
      case 'heading': {
        ...
        const content = heading.tokens ? renderInlineTokens(heading.tokens as Tokens.Generic[]) : heading.text;
      ...
      case 'paragraph': {
        ...
        const content = para.tokens ? renderInlineTokens(para.tokens as Tokens.Generic[]) : para.text;
        if (!content || (Array.isArray(content) && content.length === 0)) return null;
        return (
      ...
      case 'code': {
        ...
        if (lang === 'flow' || lang === 'flowchart' || lang === 'steps') {
          const steps = code.text
            ...
            .map((s) => s.trim())
            .filter((s) => s.length > 0);
          const direction = lang === 'steps' ? 'vertical' : code.text.includes('-->') ? 'horizontal' : 'vertical';
          return (
            <SimpleFlow key={index} steps={steps} theme={theme} direction={direction as 'vertical' | 'horizontal'} />
          );
        }

        return <CodeBlock key={index} code={code.text} language={code.lang} theme={theme} />;
      }
      ...
      case 'list': {
        ...
        return (
          <View key={index} style={styles.list}>
            {list.items
              .map((item, i) => {
                const itemContent = item.tokens ? renderInlineTokens(item.tokens as Tokens.Generic[]) : item.text;
                const hasContent =
                  itemContent &&
                ...
                return (
                  <View key={i} style={styles.listItem}>
                    ...
                    <Text style={styles.listItemContent}>{itemContent}</Text>
                  </View>
                );
              })
              .filter(Boolean)}

...
test('the staged release payload and the verifier share one plugin member list', () => {
  ...
  // Nine members, enumerated explicitly (never a glob) in exactly one place:
  // five metadata files, the manager and two sub-row bundles, and the one
  // client bundle.
  expect(DSH_PLUGIN_MEMBERS).toHaveLength(9);

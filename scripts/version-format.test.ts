...
describe('manifest version formatting', () => {
  ...
  /**
   * `plugins/genie/package.json` used to be regenerated by the deleted
   * `scripts/build.js`; it is now a reviewed, committed Orca-payload manifest.
   * Its shape is asserted directly so a hand edit cannot silently drop the MIT
   * license or add a runtime dependency to the shipped payload.
   */
  test('the committed plugin package manifest keeps its reviewed MIT metadata', () => {
    ...
    expect(`${JSON.stringify(tracked, null, 2)}\n`).toBe(source);
  });

...
describe('runtime plugin state', () => {
  ...
  test('fresh Codex home installs agents, migrates dead OTel, and runs idempotent marketplace/plugin commands', () => {
    // The shipped `codex-agents/` payload left with the Codex plugin subsystem,
    // so the role-agent convergence is driven from an explicit fixture bundle.
    const fixtureBundle = makeInstalledHome();
    ...
    expect(readdirSync(join(codexHome, 'agents')).filter((name) => name.startsWith('genie-')).length).toBe(1);
    ...
    expect(calls).toContain(`codex plugin marketplace add ${fixtureBundle} --json`);
...
describe('resolveBundleRoot', () => {
  ...
  test('a GENIE_HOME without the plugin payload falls through to the source checkout', () => {
    ...
    // Under bun test the import.meta fallback is the repo checkout, which carries plugins/genie.
...
describe('codex repair convergence (stale marketplace root / stale installed plugin)', () => {
  ...
  test('marketplace registered from a different source is repointed with bounded subprocesses', () => {
    const bundleRoot = makeInstalledHome();
    const calls: string[] = [];

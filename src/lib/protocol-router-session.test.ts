...
let spawnCalls: Array<{ continueName?: string; senderSession?: string }> = [];
...
beforeEach(async () => {
  ...
  __setProtocolRouterTestDeps({
    ...
    spawnWorkerFromTemplate: async (_template, continueName, senderSession) => {
      spawnCalls.push({ continueName, senderSession });
      spawnAttempted = true;
...
describe('sendMessage session isolation', () => {
  test('auto-spawns a suspended worker from the same session', async () => {
    ...
    expect(spawnCalls).toEqual([{ continueName: 'alpha-implementor', senderSession: 'project-a' }]);
  });
...
describe('sendMessage recipient resolution', () => {
  ...
  test('resolves worker candidate by role when registry.get misses', async () => {
    ...
    expect(result.delivered).toBe(true);
    // The spawn should use the derived continue name (team-role)
    expect(spawnCalls.length).toBeGreaterThan(0);
    expect(spawnCalls[0]?.continueName).toBe('alpha-fixer');
  });
  ...
  test('uses template lastSessionId when worker has no claudeSessionId', async () => {
    ...
    expect(result.delivered).toBe(true);
    expect(spawnCalls[0]?.continueName).toBe('alpha-engineer');
  });

  test('codex provider template does not pass continueName', async () => {
    const worker = {
      id: 'codex-worker',
      paneId: '%90',
      session: 'project-a',
    ...
    expect(result.delivered).toBe(true);
    // Codex provider should not pass continue name
    expect(spawnCalls[0]?.continueName).toBeUndefined();
  });
});

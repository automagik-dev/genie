...
/**
 * The begin line used when `install` CREATED the target file, instead of
 * appending to one the user already had. It is the only durable evidence of
 * that fact: the two cases produce byte-identical content otherwise, so without
 * it uninstall could not tell a file it must delete from a file the user keeps
 * deliberately empty. Uninstall removes the FILE when this fence's block leaves
 * nothing behind (dogfood r5 Z9: every install/uninstall cycle left a 0-byte
 * ~/.tmux.conf in a HOME that had none before).
 */
const FENCE_BEGIN_OWNED_FILE =
  '# >>> genie shortcuts — managed block; genie shortcuts install created this file and uninstall removes it whole (generated by genie-cli) >>>';
...
async function promptInstallTo(
  ...
  // A file genie is about to create gets the owning fence and no leading blank
  // line; a file the user already had keeps the plain fence, so uninstall never
  // deletes something it did not create.
  const creating = !existsSync(filePath);
  const begin = creating ? FENCE_BEGIN_OWNED_FILE : `\n${FENCE_BEGIN}`;
  appendFileSync(filePath, `${begin}\n${contentFn()}${FENCE_END}\n`);
...
export function removeMarkedContent(filePath: string, marker: string): 'removed' | 'absent' | 'modified' {
  ...
  const begin = lines.findIndex((line) => line === FENCE_BEGIN || line === FENCE_BEGIN_OWNED_FILE);
  if (begin !== -1) {
    const end = lines.indexOf(FENCE_END, begin + 1);
    if (end !== -1) {
      return spliceOutBlock(filePath, lines, begin, end + 1, lines[begin] === FENCE_BEGIN_OWNED_FILE);
    }
  }
...
/**
 * Write the file back without lines [blockStart, blockEnd) and the blank line
 * `install` prepended. When the block leaves nothing behind AND `ownedFile` says
 * `install` created this file, the file itself is removed: a config genie
 * invented must not survive its own uninstall as a 0-byte husk. A file the user
 * already had — even an empty one — is only ever emptied, never deleted.
 */
function spliceOutBlock(
  filePath: string,
  lines: string[],
  blockStart: number,
  blockEnd: number,
...
async function promptUninstallFrom(
  ...
  if (result === 'removed') {
    ...
    console.log(`✅ Removed from ${filePath}${deleted ? ' (file deleted — install created it)' : ''}`);
    return { outcome: 'applied', leftUntouched: false };
  }
  if (result === 'modified') {

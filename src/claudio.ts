...
import { type LaunchOptions, launchDefaultProfile, launchProfile } from './commands/launch.js';
import { configCommand, modelsCommand } from './commands/models.js';
import {
  profilesAddCommand,
...
const claudePassthroughArgs: string[] = separatorIndex !== -1 ? process.argv.slice(separatorIndex + 1) : [];
const claudioArgs = separatorIndex !== -1 ? process.argv.slice(0, separatorIndex) : process.argv;
...
if (claudePassthroughArgs.length > 0 && claudioArgs.length === 2) {
  ...
  program.name('claudio').description('Launch Claude Code with custom LLM router profiles').version(VERSION);
